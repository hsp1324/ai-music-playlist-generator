...
def auto_publish_single(client: httpx.Client, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not cover_path and not args.release_id and not args.allow_generated_draft_cover:
        raise RuntimeError(
            ...
            "Generate a final 16:9 cover image with only the lower-left channel-name watermark first, then pass --cover ABSOLUTE_FINAL_COVER_IMAGE_PATH."
        )
    if not thumbnail_path and not args.release_id and not args.allow_cover_as_thumbnail:
        raise RuntimeError(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    auto_single_parser.add_argument("--cover", default="", help="Required final 16:9 cover image with only the lower-left channel-name watermark unless an uploaded final cover already exists on the release.")
