...
- OpenClaw machine: creates workspaces, generates/uploads audio, cover, thumbnail, and required visual source. Moving-video channels use a short loop video; HaruHaru uses a still-image render source.
...
- OpenClaw should still call `scripts/openclaw-release render-video` after audio/cover/thumbnail and the required visual source are ready. Moving-video channels require a loop video first. HaruHaru requires no loop video and should render from the still cover with `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. That creates the queued video job; render workers do the actual MP4 work. For vocal releases, OpenClaw should upload clean lyrics with the audio. The app auto-burns original-language line lyrics into the MP4 when singable lyrics are present and creates YouTube CC captions at publish time from the same saved lyrics, using faster-whisper line timing and Codex translations. Lyric placement is channel policy: 불송 / The New Verse / Bulsong uses the centered `center-breath-serif` burn-in style; HaruHaru uses `editorial-lower-left`; every other channel uses auto policy so lyrics do not collide with the spectrum overlay. Cinematic Pulse now uses the normal loop-video path by default: photorealistic first-frame art, provider loop video, final render `720p`, and `bars` spectrum unless a human explicitly asks for still-image fallback or higher resolution.
