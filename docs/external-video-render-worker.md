...
- Still-image render jobs require updated render workers that advertise `still_image_video_fps: 30`. Older workers should not claim these jobs, because the previous still-image ffmpeg path produced a 2fps base stream and visibly choppy spectrum motion. Still-image mode is now only a human-approved fallback for specific releases; normal Cinematic Pulse work uses loop-video mode.
