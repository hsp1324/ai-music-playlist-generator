...
- The main VM pauses new render claims before disk pressure reaches the cleanup threshold. With the default `AIMP_LOCAL_VIDEO_CLEANUP_DISK_THRESHOLD_PERCENT=80` and `AIMP_RENDER_WORKER_CLAIM_DISK_SAFETY_MARGIN_PERCENT=5`, workers receive no new job once storage is at or above roughly 75%. Existing claimed renders may finish/upload, but the queue will not hand out more work until usage drops.
