...
- OpenClaw machine: creates workspaces, generates/uploads audio, cover, thumbnail, and required visual source. Moving-video channels use a short loop video; HaruHaru, sundaze, Solwave Radio, and Club Bloom use a still-image render source.
...
- Oracle workers default to claiming queued playlists without singable lyrics or 720p resolution first, preserving queue order inside that preferred group. If no preferred job is available, they fall back to the oldest claimable queued job. Set `AIMP_RENDER_WORKER_PREFER_NO_LYRICS=false` or pass `--no-prefer-no-lyrics` to disable the no-lyrics part of that behavior.
- Workers that must never handle lyric/caption renders should set `AIMP_RENDER_WORKER_ONLY_NO_LYRICS=true` or pass `--only-no-lyrics`. This advertises a strict no-lyrics capability, hides Whisper/CJK lyric capabilities, and skips releases with singable lyrics at claim time.
...
- The main VM pauses new render claims at the cleanup threshold. With the default `AIMP_LOCAL_VIDEO_CLEANUP_DISK_THRESHOLD_PERCENT=80` and `AIMP_RENDER_WORKER_CLAIM_DISK_SAFETY_MARGIN_PERCENT=0`, workers receive no new job once storage is at or above roughly 80%. Existing claimed renders may finish/upload, but the queue will not hand out more work until usage drops. Cleanup first removes public-retention-expired uploaded final MP4 files; if usage remains above threshold and `AIMP_LOCAL_VIDEO_CLEANUP_EMERGENCY_ENABLED=true`, it removes the oldest already-uploaded local final MP4 files, including scheduled/private YouTube uploads, without deleting the YouTube video or source assets.
...
- Temporary `503` / database-busy responses during progress updates, upload-status checks, chunk upload, or completion are retryable. The worker should keep the same `--worker-id`, ask upload-status again, and resume from the server's current byte offset instead of treating the render as failed.
...
- The main VM uses SQLite WAL mode and short DB busy timeouts so render-worker heartbeats/uploads do not hold the web UI indefinitely. Keep worker API calls retry-friendly; transient database contention is an operational retry condition, not a reason to abandon a completed local render.
- OpenClaw should still call `scripts/openclaw-release render-video` after audio/cover/thumbnail and the required visual source are ready. Moving-video channels require a loop video first. HaruHaru, sundaze, and Solwave Radio require no loop video and should render from the still cover with `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. Club Bloom also requires no loop video and should render from the still cover with `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars` without lyric overlay. That creates the queued video job; render workers do the actual MP4 work. For vocal releases, OpenClaw should upload clean lyrics with the audio. The app auto-burns original-language line lyrics into the MP4 when singable lyrics are present. Lyric placement is channel policy: 불송 / The New Verse / Bulsong uses the centered `center-breath-serif` burn-in style; HaruHaru, sundaze, and Solwave Radio use `editorial-lower-left`; every other lyric channel uses auto policy so lyrics do not collide with the spectrum overlay. Cinematic Pulse now uses the normal loop-video path by default: photorealistic first-frame art, provider loop video, final render `720p`, and `bars` spectrum unless a human explicitly asks for still-image fallback or higher resolution.
- Still-image render jobs require updated render workers that advertise `still_image_video_fps: 30`. Older workers should not claim these jobs, because the previous still-image ffmpeg path produced a 2fps base stream and visibly choppy spectrum motion. Still-image mode is now the normal HaruHaru, sundaze, Solwave Radio, and Club Bloom path and otherwise a human-approved fallback for specific releases; normal Cinematic Pulse work uses loop-video mode.
- `The New Verse` and `Bulsong` are legacy aliases only. The current Buddhist channel title is `불송`.
