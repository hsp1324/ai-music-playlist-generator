...
- OpenClaw should still call `scripts/openclaw-release render-video` after audio/cover/thumbnail/loop-video are ready. That creates the queued video job; render workers do the actual MP4 work. For vocal releases, OpenClaw should upload clean lyrics with the audio, but should not pass `--lyrics-overlay` by default. The main VM creates YouTube CC captions at publish time from the saved lyrics, using faster-whisper line timing and Codex translations. `--lyrics-overlay` is only for explicit burned-in on-video lyric tests or approved releases. Cinematic Pulse is the still-image high-resolution exception: OpenClaw queues it with `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 2k --video-spectrum-overlay-style bars`, so no loop-video asset is required.
