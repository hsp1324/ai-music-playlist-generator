...
- OpenClaw should still call `scripts/openclaw-release render-video` after audio/cover/thumbnail/loop-video are ready. That creates the queued video job; render workers do the actual MP4 work. For vocal releases that should show on-screen lyrics, add `--lyrics-overlay`; the worker uses faster-whisper on the rendered audio to build line-level `lyric_cues`, then burns those subtitles into the final MP4. Cinematic Pulse is the still-image high-resolution exception: OpenClaw queues it with `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 2k --video-spectrum-overlay-style bars`, so no loop-video asset is required.
