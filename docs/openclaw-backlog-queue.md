...
While doing Suno, Gemini, uploads, metadata, or publish work, refresh the lock every 1-2 minutes:

```bash
scripts/openclaw-release openclaw-lock-heartbeat \
  --run-id "$RUN_ID" \
  --operation "suno-or-gemini-or-publish" \
...
5. Upload final cover, YouTube thumbnail, and required visual source as soon as each asset is ready; these assets do not need to wait for the app audio render. Approve the cover after upload. Moving-video channels need a Gemini loop video before `render-video` only when Gemini can produce one; HaruHaru, sundaze, Solwave Radio, and Club Bloom intentionally have no loop video and should use the still cover image. After the approved cover and required visual source are present, `render-video` may be called even while audio render is still queued/running. The app stores that as a pending video-render request, then automatically creates the final `build_video` job as soon as rendered audio is ready. Use Gemini only for moving-video channels. Do not use Dreamina, Seedance, or CapCut. Before clicking Gemini Generate, run `scripts/openclaw-release provider-video-start --release-id RELEASE_ID --first-frame FIRST_FRAME --provider gemini`; if Gemini is already running for the same release and first-frame, wait and heartbeat the OpenClaw lock. If Gemini is unavailable, on cooldown, blocked after safe retries, or still has no usable MP4 after a 20 minute wait, do not defer only for video generation; use the approved still-image fallback by passing `--allow-still-image-video --video-render-source-mode still_image` with the channel's normal spectrum/lyrics flags, then render/publish. For Cinematic Pulse, still try for photorealistic cinematic cover/first-frame art plus a subtle Gemini loop video first, but fall back to still-image render when Gemini cannot be used.
...
- For visual-only repair, replace the final cover, YouTube thumbnail, and required visual source together, then approve the cover and queue a fresh video render. For moving-video channels, use Gemini only for the loop video. If Gemini is on cooldown, unavailable, blocked after safe retries, or cannot create a usable repair clip within the wait window, do not defer only for video generation; use the approved still-image fallback with the repaired cover/first-frame and queue `still_image` render. For HaruHaru, sundaze, Solwave Radio, and Club Bloom visual repair, use the photorealistic still-image package and `still_image` render. After render completion, update or re-publish through the app's normal YouTube flow.
- Do not use Dreamina, Seedance, or CapCut because of face detection, moderation, payment, quota, CAPTCHA, browser automation, or Gemini cooldown issues. Either make the clip in Gemini, or use the approved still-image fallback, then continue with render/publish.
