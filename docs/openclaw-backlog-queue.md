...
4. Generate and upload roughly 10 minutes of new approved audio for normal non-scripture channels, or roughly 40 minutes for BibliaCanto/불송 passage-based releases. For Soft Hour Radio, the new 10-minute block must be solo piano only and uploaded first; the app extends the back half with previous same-channel similar tracks, preferring solo-piano tracks first and falling back to similar existing Soft Hour music if there are not enough piano tracks to approach one hour. When audio render is queued, the web app automatically tries to extend non-scripture base blocks to roughly 60 minutes by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube videos; if reuse is limited, anything over 40 minutes is publishable. BibliaCanto and 불송 are excluded from normal back-half reuse. During the trial period, final video repeat is disabled and render workers upload the base block only.
...
- Current human repair note from 2026-05-15, updated 2026-06-05: `Club Bloom` should use stronger, more click-stopping photorealistic nightlife still images by default. Remake only cover/thumbnail visuals when needed, then re-render from the existing music with `still_image`; do not create a provider loop video unless the human explicitly asks for motion.
