...
4. Generate and upload roughly 10 minutes of new approved audio for normal non-scripture channels, or roughly 60 minutes for BibliaCanto/불송 passage-based releases. For Soft Hour Radio, the new 10-minute block must be solo piano only and uploaded first; the app extends the back half with previous same-channel similar tracks, preferring solo-piano tracks first and falling back to similar existing Soft Hour music if there are not enough piano tracks to approach one hour. When audio render is queued, the web app automatically tries to extend non-scripture base blocks to roughly 60 minutes by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube videos; if reuse is limited or the matching pool is exhausted, publish can still proceed below one hour instead of blocking. BibliaCanto and 불송 are excluded from normal back-half reuse, so OpenClaw should aim for about 3600 seconds of new same-passage/same-source audio before render. During the trial period, final video repeat is disabled and render workers upload the base block only.
