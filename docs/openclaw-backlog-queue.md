...
6. First finish existing releases that are already past video render:
    ...
    - `publish_ready`: approve publish through the app, but use `scripts/openclaw-release publish-release --no-wait` during continuous automation so the app upload worker owns the YouTube upload. Do not finish the lock as `blocked` only because the rendered playlist is shorter than one hour. Production should aim for one hour or longer before render, but a release that is already rendered and metadata-approved should publish instead of looping on duration top-up.
...
7. Before creating a new release, inspect existing unfinished workspaces that are not yet past video render, especially `collecting`, `audio_ready`, `render_required`, or channel-unknown workspaces with app-visible tracks/assets. These may be interrupted OpenClaw runs with Suno audio, cover, thumbnail, or loop-video files already prepared locally. Search the local OpenClaw workspace/logs by release id and title, then resume that workspace first when real assets exist: set the target YouTube channel if missing, upload missing tracks/assets, render audio, approve cover, and queue video render. For HaruHaru, sundaze, Solwave Radio, and Club Bloom, resume with still-image render flags instead of looking for a loop video. Do not stop the backlog pass only because an empty duplicate shell exists: if a `collecting` workspace has 0 tracks, no rendered audio, no cover, no thumbnail, no loop video, and the same title/channel already has an uploaded/scheduled release, the app backlog summary auto-archives or skips that empty shell. Continue creating/resuming the next eligible release. Similar titles or already published similar concepts are not blockers by themselves; avoid obvious repetition when choosing a new concept, but keep production moving.
...
Use `--status blocked` if captcha, login, missing API, account verification, or actual Suno credit exhaustion prevents progress. Do not use `blocked` for YouTube API upload quota/rate-limit exhaustion; that is deferred publish work, so finish the lock as `completed` after moving to the next eligible workspace or after reporting that no eligible work remains. If Suno credits are temporarily unavailable again in the future, do not idle when reusable tracks exist: build reuse-only releases from existing approved similar tracks and finish the lock as `completed` after queuing render or after selecting another eligible workspace.
...
3. Create the Playlist Release before Suno generation or existing-track reuse selection.
...
4. Generate and upload roughly 10 minutes of new approved audio for normal playlist channels and 불송, or roughly 60 minutes for BibliaCanto passage-based releases. For Soft Hour Radio, the new 10-minute block must be solo piano only and uploaded first; the app extends the back half with previous same-channel similar tracks, preferring solo-piano tracks first and falling back to similar existing Soft Hour music if there are not enough piano tracks to approach one hour. For 불송, the new lead block must be uploaded first and should use a clear modern Korean vocal direction, but rap, hip-hop, R&B, soul, and quiet piano vocal ballads may share the same mainstream 불송 vocal pool when the emotional mood fits. The app extends with previous same-channel compatible 불송 vocal tracks, liked first, and must not mix unrelated jazz, trot, guitar-folk/acoustic, cinematic, instrumental BGM, no-vocal piano, or meditation tracks just to hit one hour. New default 불송 lyrics and public metadata avoid overt Buddhist/source vocabulary unless the human explicitly asks for source-explicit wording. When audio render is queued, the web app automatically tries to extend eligible base blocks to roughly 60 minutes by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube videos; if reuse is limited or the matching pool is exhausted, publish can still proceed below one hour instead of blocking. BibliaCanto is excluded from normal back-half reuse, so OpenClaw should aim for about 3600 seconds of new same-passage/same-source audio before render. During the trial period, final video repeat is disabled and render workers upload the base block only.
   - Suno credit status: the 2026-06-05 no-new-audio restriction is lifted after the human added 500 credits. OpenClaw may generate new Suno audio again, but should avoid burning credits on retries: use only the usable tracks already made, then fill the remaining duration with existing approved same-channel, similar-style tracks.
   - For the reuse fill, choose tracks in the same truthful genre lane, search with `scripts/openclaw-release search-tracks --status-filter approved`, attach selected tracks with `scripts/openclaw-release reuse-track`, and keep generated and reused tracks coherent. Prefer liked tracks, exclude disliked/copyright-blocked/reuse-disabled tracks, and do not fill a hip-hop/R&B/trap release with unrelated city-pop, generic pop, or BGM tracks.
5. Upload final cover, YouTube thumbnail, and required visual source as soon as each asset is ready; these assets do not need to wait for the app audio render. Approve the cover after upload. Moving-video channels need the required loop video before `render-video`; HaruHaru, sundaze, Solwave Radio, and Club Bloom intentionally have no loop video and should use the still cover image. After the approved cover and required visual source are present, `render-video` may be called even while audio render is still queued/running. The app stores that as a pending video-render request, then automatically creates the final `build_video` job as soon as rendered audio is ready. Try Gemini first for moving-video channels, then use Dreamina/Seedance when Gemini is on cooldown, unavailable, blocked after retries, or still has no usable MP4 after a 20 minute wait. Before clicking Generate in any provider, run `scripts/openclaw-release provider-video-start --release-id RELEASE_ID --first-frame FIRST_FRAME --provider PROVIDER`; if Gemini is already running for the same release and first-frame, wait and heartbeat the OpenClaw lock instead of starting Dreamina/Seedance. If Dreamina/Seedance cannot create the clip because of credit, account, quota, CAPTCHA, browser, or provider failure, try Gemini again when quota is available. If Gemini has already used all 3 successful videos in the current 24 hour window, do not defer only for video generation; use the approved still-image fallback by passing `--allow-still-image-video --video-render-source-mode still_image` with the channel's normal spectrum/lyrics flags, then render/publish. For Cinematic Pulse, still try for photorealistic cinematic cover/first-frame art plus a subtle loop video first, but fall back to still-image render when Gemini quota is exhausted and Dreamina/Seedance cannot be used.
6. Before approving Cinematic Pulse metadata or publish, compare the final track list titles/styles against the title, description, tags, cover phrase, thumbnail phrase, and visual mood. If cute/playful anime-game/arcade/item-shop/theme-park BGM is packaged as dark fantasy, epic battle, final battle, trailer music, or cinematic orchestra, stop and correct the release instead of publishing.
7. Render audio with `scripts/openclaw-release render-audio --release-id RELEASE_ID --randomize-order`. For Soft Hour Radio, order randomization must keep newly generated solo-piano tracks in the front lead block and reused tracks after them; the app preserves this when it can identify reused back-half tracks, but OpenClaw must not manually move reused tracks ahead of new piano tracks.
8. Approve the uploaded cover with `scripts/openclaw-release approve-cover --release-id RELEASE_ID`.
9. Queue video render with `scripts/openclaw-release render-video --release-id RELEASE_ID --video-spectrum-overlay-style PRESET`.
10. Choose the visualizer preset that fits the release art; do not rely on the default when the visual mood clearly calls for another preset. For `HaruHaru`, queue `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. For `Club Bloom`, queue `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars` without lyric overlay. For `Cinematic Pulse`, queue `--video-render-source-mode loop_video --video-render-resolution 720p --video-spectrum-overlay-style bars` unless a human explicitly asks for a still-image fallback or higher resolution.
11. Do not pass `--wait` in normal automation. Do not approve metadata or publish until the app later asks again after external render completion.
12. Release the OpenClaw lock and report the queued release id.
...
- For visual-only repair, replace the final cover, YouTube thumbnail, and required visual source together, then approve the cover and queue a fresh video render. For moving-video channels, try Gemini first for the loop video, then use Dreamina/Seedance when Gemini is on cooldown, unavailable, or blocked after retries. If Dreamina/Seedance cannot create the repair clip and Gemini quota is exhausted, do not defer only for video generation; use the approved still-image fallback with the repaired cover/first-frame and queue `still_image` render. For HaruHaru, sundaze, Solwave Radio, and Club Bloom visual repair, use the photorealistic still-image package and `still_image` render. After render completion, update or re-publish through the app's normal YouTube flow.
- If Dreamina/Seedance fails because of face detection, moderation, payment, quota, CAPTCHA, or browser automation issues, do not make an interim local motion loop. Either make the clip in Gemini, or use the approved still-image fallback if Gemini quota is exhausted, then continue with render/publish.
- For HaruHaru photorealistic work, visible adult faces are allowed and preferred when natural, but avoid tight straight-on AI-beauty close-ups. Use clearly adult women/men or tasteful adult couple/friend scenes that feel like ordinary friend-taken travel/cafe snapshots: side or three-quarter angles, candid smiles/laughs, medium or farther framing, slight phone-photo imperfection, seaside/flower/cafe/street settings. Avoid minors, school uniforms, celebrity/idol likenesses, over-retouched doll faces, and explicit framing. Do not spend credits on provider loop videos unless the human explicitly asks for motion.
...
- Current human repair note from 2026-05-15, updated 2026-06-11: `Club Bloom` should use stronger, more click-stopping HaruHaru-style photorealistic friend-taken smartphone/Instagram nightlife still images by default, with club/bar/lounge/festival backgrounds and attractive clearly adult women in revealing YouTube-safe club fashion. Remake only cover/thumbnail visuals when needed, then re-render from the existing music with `still_image`; do not create a provider loop video unless the human explicitly asks for motion.
