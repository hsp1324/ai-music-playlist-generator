...
Use this profile only after channel selection returns `The New Verse`, or when the human explicitly asks for Buddhist scripture-inspired music. The YouTube channel title may be renamed later; until then, use the current connected channel title.
...
- Do not route Bible, Gospel, Old Testament, or New Testament worship here. Those now upload to `The Old Verse`.
...
- The cover must include only a large lower-left `The New Verse` channel brand label until the human renames the channel.
...
- Add `THE NEW VERSE` as the brand line until the channel is renamed.
...
- Preserve the large lower-left `The New Verse` text exactly for the full clip.
- Queue final render with `--video-spectrum-overlay-style none`. The New Verse must not use app-rendered spectrum bars, radial/multiwave/pulse visualizers, waveform overlays, dots, particles, or equalizer graphics.
