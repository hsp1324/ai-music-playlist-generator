...
- The final moment should keep the same crop, framing, camera distance, lighting, palette, and subject placement. The app handles smooth repetition with a 1.5 second crossfade, so do not force the model to zoom out or return the camera to the opening frame.
