...
- New 불송 releases are diversity-first. Hip-hop remains valid, but OpenClaw should also rotate into R&B, trip-hop, glitch-hop, drum and bass, hyperpop, art-pop, psychedelic soul, avant-jazz rap, spoken-word rap, new-age electronica, cinematic trap, and other coherent modern lanes when they fit the internal source/theme. Keep passage/theme/genre phrases such as `삼도 힙합`, `법구경`, `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩` out of new public metadata and visual text unless the human explicitly asks for source-explicit wording in the current request. Never use `불송` as visual text, and avoid obscure coined genre labels.
