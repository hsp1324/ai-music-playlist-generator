...
- Apply the channel visual signature to both static images: exactly three people seen from behind, walking away from camera into the scene. The cover should be the clean version; the thumbnail must be generated from that final cover as a reference/edit derivative, using the same composition plus readable click text and channel branding. In the thumbnail, keep the three people centered; text must fit around the composition rather than moving the people sideways.
