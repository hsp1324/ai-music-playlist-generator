...
- Do not ask Suno to fade out, slow fade, dissolve, or end by lowering volume. A slightly longer complete song is better than a track that sounds cut off. Prompts, style strings, lyrics, and bracketed metatags should say `no fade-out`, `complete ending`, `final cadence`, `resolved ending`, or `hard ending` where appropriate.
