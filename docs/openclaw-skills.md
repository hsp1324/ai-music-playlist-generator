...
- `cover`: video visual shown during playback. It should look good for the full video duration and must include only the selected channel name as a lower-left watermark used as the Dreamina/Seedance first-frame text reference. Make every channel watermark medium-sized and readable on mobile, not tiny; target roughly 7-9% of the image width.
