...
- All static images and Dreamina/Seedance/Gemini loop clips must look animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. Soft Hour Radio, HaruHaru, sundaze, Solwave Radio, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; Soft Hour Radio uses photorealistic quiet BGM environments and a static locked-off camera with no camera movement in provider loop videos; HaruHaru, sundaze, and Solwave Radio use photorealistic still-image lifestyle visuals by default, with sundaze/Solwave specifically using a casual friend-taken smartphone/Instagram look; 불송 uses a respectful photorealistic Buddha-reading-sutra scene by default and only uses gentle animation when the human explicitly asks or the release concept clearly requires it; Cinematic Pulse should use restrained photorealistic cinematic motion in the provider loop video rather than animated/anime styling.
...
- Do not substitute a locally synthesized motion loop, app-rendered still-image animation, Ken Burns pan/zoom, or other non-provider workaround when Dreamina/Seedance fails. Dreamina/Seedance failure should route to Gemini if quota is available; if Gemini quota is exhausted, defer that release and continue with the next eligible release. HaruHaru, sundaze, and Solwave Radio are not fallbacks: they deliberately use app still-image rendering from the uploaded cover.
- Keep these assets separate for normal moving-video channels: `--thumbnail` is the click image, `--cover` is the playback visual/first frame, and `--loop-video` is the short moving visual used inside the rendered video. Do not put channel names or logos on any of them. For HaruHaru, upload `--cover` and `--thumbnail` only, then render with `--allow-still-image-video --video-render-source-mode still_image`; do not upload `--loop-video` unless the human explicitly asks for motion. HaruHaru cover/thumbnail prompts should explicitly avoid professional photoshoot language, idol styling, and tight frontal portrait language; use friend-taken Hongdae/Seoul street snapshot language with hip adult streetwear, slight motion/focus softness, and Korean hip-hop/R&B mood instead. For Solwave Radio, upload `--cover` and `--thumbnail` only, then render with `--allow-still-image-video --video-render-source-mode still_image --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`; do not upload `--loop-video` unless the human explicitly asks for motion. Solwave cover/thumbnail prompts should explicitly avoid professional photographer, studio portrait, fashion-campaign, and AI-beauty headshot language; use friend-taken night-out/travel/cafe phone snapshot language instead. For `불송`, use one cover/thumbnail/first-frame package showing a serene Buddha / Buddha-inspired figure listening to music while reading or holding an open sutra, with a short Korean passage/theme + style phrase when useful.
...
- A final 16:9 cover image is required. For moving-video releases, this cover also acts as the Gemini/Dreamina/Seedance first-frame reference. For HaruHaru, sundaze, and Solwave Radio, it is the still-image render source. Do not put the selected channel name, a channel logo, or a brand line on it. Use only a short style, genre, use-case, or passage phrase when visual text is useful.
- A separate YouTube thumbnail image is required for normal channels. Most channels use readable click text. For HaruHaru, create a text-free photorealistic lifestyle thumbnail derived from the cover unless the human explicitly asks for text. For sundaze, create a photorealistic friend-taken phone-photo English/American lifestyle thumbnail derived from the cover with one short integrated English-pop lane phrase when useful. For Solwave Radio, create a photorealistic friend-taken phone-photo lifestyle thumbnail derived from the cover with one short integrated Latin/Spanish lane phrase when useful. For `불송`, use the same Buddha-reading-sutra cover/first-frame package as the thumbnail, usually with the short Korean passage/theme + style phrase. For J-pop/Japan singles, use natural J-pop lane text such as `J-POP`, `CITY POP`, or `ANIME POP`, not the channel name. For Storylight OST, Cinematic Pulse, Club Bloom, and BibliaCanto, use the selected channel profile and song concept for thumbnail wording; there is no fixed recurring channel-brand signature. Do not add duration text or badges such as `1 HOUR`, `60 MIN`, or `1時間`. Any thumbnail text must have a transparent background: no black boxes, semi-transparent dark panels, white or colored rectangles, gradient scrims, stickers, badges, pills, capsules, or filled label shapes.
- Apply the selected channel profile to both static images. For J-pop/Japan/Tokyo Daydream Radio singles, use the Tokyo profile. For Soft Hour/solo-piano BGM singles, use the Soft Hour profile. For HaruHaru, sundaze, Solwave Radio, Storylight OST, Cinematic Pulse, Club Bloom, BibliaCanto, or 불송, use that channel's profile and let the song concept drive the scene. The thumbnail should be generated from the final cover as a reference/edit derivative with the same composition plus readable click text where the profile calls for it. HaruHaru thumbnails should normally keep the same photorealistic scene without text. sundaze thumbnails should keep the same photorealistic English/American friend-taken phone-photo scene with at most one integrated lane phrase. Solwave Radio thumbnails should keep the same photorealistic Latin/Spanish friend-taken phone-photo scene with at most one integrated lane phrase. In thumbnails, keep the main requested subject centered; text must fit around the composition rather than moving the subject sideways.
...
- Keep the visual style animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, sundaze, Solwave Radio, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic friend-taken Hongdae/Seoul hip streetwear still images by default, sundaze and Solwave use a casual friend-taken smartphone/Instagram look, and 불송 uses a respectful photorealistic Buddha-reading-sutra package by default.
...
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail has readable click text except on HaruHaru, where it should normally be text-free. The cover/loop video use the cleaner first-frame composition. None should contain channel names. The loop video must remain free of subtitles, lyrics, UI, logos, title sentences, duration text, and unrelated words. For HaruHaru, upload only the final cover and text-free thumbnail, then render from `still_image` with lower-left lyrics and lower-right spectrum. For sundaze and Solwave Radio, upload only the final cover and photorealistic friend-taken phone-photo thumbnail, then render from `still_image` with lower-left lyrics and lower-right spectrum.
...
`--loop-video` is required for normal moving-video publish automation. If it is missing, stop and create/download the Gemini/Dreamina/Seedance clip first. Do not create a local motion-loop workaround. For HaruHaru, sundaze, and Solwave Radio, omit `--loop-video` and pass `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. Use `--allow-still-image-video` on other channels only when the human explicitly accepts a static-cover fallback.
...
- Create roughly `600` seconds / 10 minutes of new approved audio for non-scripture channels except `Storylight OST`. If Suno is unstable, stop generation, upload only the tracks already created, and call audio render instead of burning credits on retries. For Soft Hour Radio, this new lead block must be solo piano only and uploaded first. The app will try to extend the base block to roughly `3600` seconds / 60 minutes by reusing tracks from the back half of previous same-channel, similar-genre YouTube uploads; for Soft Hour, it prefers similar solo-piano tracks first and then falls back to similar existing Soft Hour music if piano is not enough. Within eligible similar candidates, liked tracks are selected before neutral tracks, disliked/copyright-blocked/reuse-disabled tracks are never reused, and then least-used/recent back-half material is preferred. If similar reuse material is not available, a release over `2400` seconds / 40 minutes is still publishable.
...
- Keep every generated visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, sundaze, Solwave Radio, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic friend-taken Hongdae/Seoul hip streetwear still images by default, sundaze uses photorealistic English/American friend-taken smartphone lifestyle still images by default, Solwave Radio uses photorealistic Latin/Spanish friend-taken smartphone lifestyle still images by default, and 불송 uses a respectful photorealistic Buddha-reading-sutra package by default.
- Generate a short motion clip before running the full publish command for moving-video channels, then pass it with `--loop-video` and `--loop-video-provider gemini|dreamina|seedance`. Try Gemini first unless Gemini is on cooldown; use Dreamina/Seedance when Gemini is unavailable, has reached 3 successful videos in the last 24 hours, or cannot create a video after 10 blocked prompt attempts. If Dreamina/Seedance cannot create the clip, try Gemini again if quota is available; if Gemini quota is exhausted, defer this release and resume it first after the 24 hour Gemini cooldown. For normal HaruHaru, sundaze, and Solwave Radio releases, do not create a motion clip and do not pass `--loop-video`; render with `--allow-still-image-video --video-render-source-mode still_image`.
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail contains readable click text except on HaruHaru, where it should normally be text-free; the cover and loop video use the cleaner first-frame composition. None should contain channel names. For HaruHaru, sundaze, and Solwave Radio, verify the cover/thumbnail leave lower-left and lower-right space for app-managed lyrics and spectrum. For `불송`, use one Buddha-reading-sutra cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase when useful. Verify that Gemini/Dreamina/Seedance preserves the intended frame in moving clips.
- A Gemini/Veo provider logo or watermark in the corner is allowed and is not a reason to regenerate. Do not add any other logos or UI yourself.
...
- If the first-frame image itself appears to be blocked on a moving-video channel, regenerate a safer first-frame/cover image and then retry. If Dreamina/Seedance still cannot create the clip, try Gemini if quota is available. If Gemini quota is exhausted, defer this release, report the deferral in Slack, and continue with the next eligible release before render/publish unless the human explicitly accepts a still-image fallback. If accepted, pass `--allow-still-image-video`. HaruHaru, sundaze, and Solwave Radio already use still-image render by default, so this failure path does not apply unless the human explicitly requested motion.
- If Dreamina login, CAPTCHA, payment, face detection, or human approval blocks browser automation, do not create a local motion-loop substitute. Try Gemini if quota is available; if Gemini quota is exhausted, defer this release and move on. For HaruHaru, sundaze, and Solwave Radio, avoid this path by not creating a provider loop video in normal automation.
- For HaruHaru, sundaze, and Solwave Radio photorealistic releases, prompt for clearly adult stylish subjects with visible natural faces allowed. Do not hide faces by default, and reject/regenerate only for explicit, minor-coded, celebrity-like, policy-blocked, badly distorted, or visually unusable results.
...
Do not omit `--cover` or `--thumbnail` for normal playlist production. Do not omit `--loop-video` for moving-video channels. For HaruHaru, sundaze, and Solwave Radio, intentionally omit `--loop-video` and render with `still_image`. If any required asset is not ready, stop and report the missing asset. The app's local draft cover is only a placeholder for manual review, not acceptable for automatic YouTube upload.

Use `--randomize-order` when the uploaded playlist contains similar Suno two-output pairs next to each other. Omit it when the human already arranged a deliberate final order. For Soft Hour Radio, upload new solo-piano tracks first; the app keeps reused back-half tracks after the fresh lead block during randomization.

If the release is mainstream J-pop/Japanese pop/Tokyo pop, set the release channel to `Tokyo Daydream Radio`. If it is Korean/K-pop, use `HaruHaru`. If it is playful no-vocal Japanese-style game/anime OST, arcade-game BGM, fantasy-game BGM, cute RPG music, item-shop BGM, happy amusement park BGM, theme-park parade BGM, carousel/carnival BGM, or feel-good toy-town instrumental music, use `Storylight OST`. If it is no-vocal large-scale cinematic orchestra, movie OST, film score, trailer, battle, emotional, mystery-tension, or game-focus music, use `Cinematic Pulse`. If it is no-vocal EDM/house/techno/trance/workout/night-drive club music, use `Club Bloom`. If it is Old Testament/Bible sequence music or New Testament scripture music, use `BibliaCanto`, but make the sound modern hip-hop/R&B/K-pop-style rather than church/gospel/worship. If it is Buddhist scripture-inspired modern vocal music, use `불송`; the app schedules it daily at 07:00 Asia/Seoul. If it is an English/American pop playlist lane, use `sundaze`. If it is Latin/Spanish pop, use `Solwave Radio`. Otherwise use `Soft Hour Radio`.
...
- Do not create a long MP4 in OpenClaw. For moving-video channels, upload only the short loop clip with `--loop-video` and a provider tag via `--loop-video-provider`; the app handles the long render. For HaruHaru, sundaze, and Solwave Radio, do not upload a loop clip and let the app render from `still_image`.
- Do not add subtitles, lyric overlays, UI elements, title sentences, duration text, channel names, logos, or unrelated words inside the loop video. The selected channel profile controls the default visual action. An explicit human visual/video request overrides that channel default. If the first-frame reference has too much text, regenerate a cleaner first frame before using Dreamina/Seedance. For HaruHaru, sundaze, and Solwave Radio, keep lyrics and spectrum out of the static images; the app adds lower-left lyrics and lower-right spectrum during still-image render.
...
- The release needs a final cover/first-frame without channel names, a separate thumbnail, visual render handling, metadata approval, and app-managed upload. Moving-video channels need a short Gemini-first loop video. HaruHaru, sundaze, and Solwave Radio need `still_image` render flags instead of a loop video. Exception: `불송` can use one Buddha-reading-sutra cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase.
