...
- The background can change by genre and channel mood: Tokyo street, cafe alley, forest path, beach, moonlit road, rainy city, abstract dreamscape, cafe window, piano room, rain, candle detail, and so on.
...
- The 8 second loop video must visually match the thumbnail's scene and brand, but it should start from the clean text-free cover or a separate clean text-free first-frame image by default. Do not use the final text thumbnail as the Dreamina/Seedance first-frame reference, because generated video often makes text flicker, disappear, or reappear during the loop. If the human explicitly asks for video channel text, use a dedicated cover/first-frame with only the lower-left channel label; otherwise, if the only available first-frame image has text, regenerate a clean no-text version before Dreamina/Seedance.
- Text-in-video exception: if the human explicitly asks for channel text inside the rendered video, create the cover/first-frame with a small lower-left channel label such as `Tokyo Daydream Radio`. Let the GPT Image model design the font/lettering to match the scene, channel, and genre, but keep the exact requested spelling readable. Then ask Dreamina/Seedance to preserve that baked-in text exactly for the full 8 second clip.
- When using the text-in-video exception, do not add extra title text, lyrics, subtitles, UI, logos, or unrelated words. The lower-left channel label is the only allowed text inside the moving visual unless the human explicitly asks for more.
...
- A final 16:9 cover image is required. For moving-video releases, this cover also acts as the Dreamina/Seedance first-frame reference. By default it should be clean and text-free; if the human explicitly requests channel text inside the video, the cover/first-frame may include only that lower-left channel label.
...
- Apply [openclaw-visual-assets.md](openclaw-visual-assets.md) to both static images. For J-pop/Japan/Tokyo Daydream Radio singles, the default is exactly three people seen from behind walking away from camera into the scene. For Soft Hour/default BGM singles, use calm atmospheric visuals instead and do not use the Tokyo three-person walking signature unless explicitly requested. The cover should be the clean version; the thumbnail must be generated from that final cover as a reference/edit derivative, using the same composition plus readable click text and channel branding. In the thumbnail, keep the main channel/requested subject centered; text must fit around the composition rather than moving the subject sideways.
...
- Use the cover or a separate first-frame image as the first-frame/start-frame reference for Dreamina/Seedance so the video opening matches the thumbnail scene. By default this first frame is clean and text-free; if the human explicitly requested channel text inside the video, use the cover/first-frame that contains only the lower-left channel label and preserve it exactly. For J-pop/Japan/Tokyo Daydream Radio, the frame should show the three people from behind walking into the scene by default. For Soft Hour/default BGM, the frame should show the selected calm atmospheric scene by default. If the human requested a different visual concept, the first frame and motion prompt must follow that requested concept instead. Do not use the text thumbnail, Omni Reference, or a last-frame reference.
- The thumbnail, cover, and loop video are three different assets. The thumbnail has readable click text. The cover/loop video are text-free by default; when the human explicitly requests video text, the only allowed baked-in moving-visual text is the lower-left channel label. The loop video must still remain free of subtitles, lyrics, UI, logos, title text, and unrelated words.
...
- Use the Dreamina prompt shape from [openclaw-visual-assets.md](openclaw-visual-assets.md) for the selected channel. Tokyo/J-pop uses the three-person forward-walking prompt. Soft Hour/default BGM uses the calm environmental-motion prompt.
...
- If the human explicitly requested lower-left channel text inside the video, adjust the Dreamina prompt: `The uploaded first frame contains the exact lower-left channel label "{CHANNEL_NAME}" (for example, "Tokyo Daydream Radio"). Preserve this text exactly for the full clip. Do not rewrite, translate, blur, morph, move, hide, flicker, or change the text. Keep the text area stable; animate only the surrounding scene subtly.` Keep all other constraints.
...
- Generate or obtain the final cover image before running the full publish command, then pass it with `--cover`. Use OpenAI GPT Image models for static image creation, not Dreamina. For moving-video releases, the cover should be clean and text-free by default because it is the Dreamina/Seedance first-frame reference. If the human explicitly requests channel text in the video, the cover/first-frame may include only the lower-left channel label, with GPT-designed typography that matches the scene.
...
- Apply [openclaw-visual-assets.md](openclaw-visual-assets.md) to both images. For J-pop/Japan/Tokyo Daydream Radio playlists, the default is exactly three people seen from behind walking away from the camera into the scene. For Soft Hour/default BGM playlists, use calm atmospheric visuals instead and do not use the Tokyo three-person walking signature unless explicitly requested. Use a clean text-free version for `--cover`; use the same centered channel/requested composition plus readable title/channel text for `--thumbnail`. In thumbnails, keep the main subject centered and place text around it in negative space; never move the main subject to one side just to make room for text.
...
- The thumbnail, cover, and loop video are three different assets. The thumbnail must contain readable click text; the cover and loop video stay clean and text-free by default. If the human explicitly requests channel text inside the video, add only the lower-left channel label to the cover/first-frame and verify that Dreamina/Seedance preserves it in the 8 second clip.
- Use the cover or a separate first-frame image as the visual starting reference for Dreamina/Seedance image-to-video generation. By default this reference is clean and text-free; if the human explicitly requested video channel text, it may include only the lower-left channel label. For J-pop/Japan/Tokyo Daydream Radio, the first shot of the 8 second loop video should show the same three-person back-view walking composition as the thumbnail/cover, and the video motion should keep them walking forward away from the viewer. For Soft Hour/default BGM, the first shot should show the selected calm atmospheric scene and use slow environmental motion. If the human requested a different video concept, use that requested subject/action/composition for the cover, thumbnail, and loop video. Do not use the text thumbnail as the video first frame.
- For Dreamina motion clips, set duration/ratio/quality in Dreamina controls, not in the prompt. Use the channel-specific prompt from [openclaw-visual-assets.md](openclaw-visual-assets.md): Tokyo/J-pop uses `exactly three people seen from behind walking away from camera`; Soft Hour/default BGM uses calm environmental motion and explicitly avoids the Tokyo three-person signature. If the human requested a different visual/video concept, replace only the subject/action/camera details with that request and keep the remaining continuity/no-text/no-repeat/no-hard-cut constraints. Do not ask for a matching last frame; the app handles smooth repeat with forward crossfade rendering.
...
- Do not use Dreamina to create static cover or thumbnail images. Use OpenAI GPT Image models for static images, then use Dreamina only to animate the cover or first-frame image into an 8 second loop video. By default this image is clean and text-free; if the human explicitly requested video channel text, it may include only the lower-left channel label.
- Static cover and thumbnail images must follow [openclaw-visual-assets.md](openclaw-visual-assets.md). Tokyo/J-pop releases use the three-person back-view walking signature by default. Soft Hour/default BGM releases use calm atmospheric visuals by default and must not use the Tokyo three-person walking signature unless explicitly requested.
...
- Do not add subtitles, lyric overlays, UI elements, title text, or unrelated words inside the loop video. The default visual action is channel-specific: Tokyo/J-pop uses three people walking forward away from the viewer, while Soft Hour/default BGM uses calm atmospheric motion. An explicit human visual/video request overrides that channel default. If the first-frame reference has baked-in text other than an explicitly requested lower-left channel label, regenerate a clean/no-extra-text first frame before using Dreamina/Seedance.
