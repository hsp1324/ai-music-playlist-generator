...
- Do not put duration caps such as `less than 4 minutes` or `under 4 minutes` into Suno prompts, style strings, lyrics, or bracketed metatags unless the human explicitly asks for that cap. Those caps can cause Suno to end too early. Prompt for a full-length complete song/cue instead: at least 2 minutes, natural intro, developed verse/section flow, chorus or central motif where appropriate, bridge/breakdown when useful, and a resolved ending. Suno has no guaranteed minimum-duration field, so do not rely only on typing "2 minutes or longer" in style; use a developed song structure and verify the downloaded duration. The helper rejects playlist tracks shorter than 2:00 by default and has no default maximum duration. Complete 5+ minute tracks are allowed on every channel.
...
- `불송` YouTube channel is now for Buddhist scripture-inspired modern vocal music: Dhammapada/법구경-inspired songs, Heart Sutra/반야심경-inspired songs, emotional Buddhist pop, Buddhist jazz, mindful hip-hop, Buddhist R&B/soul, dharma neo-soul, acoustic/indie dharma pop, mindfulness songs, and modern sutra-inspired music. Use Korean lyrics by default unless the human asks otherwise. Do not route Bible/Gospel/New Testament worship there. Do not make 불송 sound like trot, ppongjjak, old Korean cabaret-pop, or old-TV trot unless the human explicitly asks for that.
...
- For `불송`, rotate modern Buddhist music lanes such as emotional Buddhist pop, Buddhist jazz, mindful hip-hop, Buddhist R&B/soul, dharma neo-soul, acoustic/indie dharma pop, warm lo-fi pop, chill pop, gentle city-pop, or cinematic meditation pop. Before Suno generation, add excluded-style blockers for `trot, Korean trot, ppongjjak, 뽕짝, 트로트, trot vocal, cabaret trot, old Korean trot, two-beat trot bounce, accordion trot, brass trot`.
...
- `cover`: video visual shown during playback and, for moving-video channels, used as the Dreamina/Seedance/Gemini first-frame reference. It should look good for the full video duration and must not include a channel name or logo. If visual text is useful, use a short style, genre, use-case, or passage phrase. For HaruHaru and Solwave Radio, this is the still-image video source.
- `thumbnail`: YouTube click thumbnail. It should usually include short readable text such as `CAFE PIANO`, `DEEP SLEEP`, `FOCUS MUSIC`, `TOKYO NIGHT`, `CITY POP`, or `J-POP`. Do not add the channel name or duration text such as `1 HOUR`, `60 MIN`, `1時間`, or time badges. Exception: HaruHaru thumbnails should normally have no text and look like natural photorealistic lifestyle/travel snapshots, not straight-on AI beauty portraits. Solwave Radio thumbnails should look like photorealistic Latin/Spanish friend-taken smartphone lifestyle photos and may use one short integrated lane phrase such as `LATIN POP`, `REGGAETON`, or `NOCHE LATINA`.
...
- Storylight OST, Cinematic Pulse, Club Bloom, BibliaCanto, 불송, and sundaze do not have fixed visual signatures yet. Use the selected channel profile and let the playlist concept drive the cover, thumbnail, and short loop video. HaruHaru defaults to photorealistic Korean lifestyle still-image cover/thumbnail packaging and renders from the still cover, with no provider loop video unless the human explicitly asks for motion. Its preferred look is a casual adult day-trip/cafe/seaside snapshot taken by a friend: natural side or three-quarter angle, medium or farther framing, visible but not face-filling subject, slight phone-photo imperfection, and no centered AI-model headshot. Solwave Radio defaults to photorealistic Latin/Spanish friend-taken smartphone lifestyle still-image cover/thumbnail packaging and renders from the still cover with app lower-left lyrics and lower-right spectrum, with no provider loop video unless the human explicitly asks for motion. Its preferred look is a casual adult night-out/travel/cafe snapshot: side or three-quarter angle, phone-glance/candid moment, medium or farther framing, slight phone-photo imperfection, no professional photographer shoot or glossy campaign. Cinematic Pulse uses photorealistic cinematic first-frame art and now also uses a subtle provider loop video by default.
...
- All static images and Dreamina/Seedance/Gemini loop clips must look animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Solwave Radio, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; HaruHaru and Solwave Radio use photorealistic still-image lifestyle visuals by default, with Solwave specifically using a casual friend-taken smartphone/Instagram look; 불송 also allows occasional cute/gentle animation when the release music lane fits it, and Cinematic Pulse should use restrained photorealistic cinematic motion in the provider loop video rather than animated/anime styling.
...
- Do not substitute a locally synthesized motion loop, app-rendered still-image animation, Ken Burns pan/zoom, or other non-provider workaround when Dreamina/Seedance fails. Dreamina/Seedance failure should route to Gemini if quota is available; if Gemini quota is exhausted, defer that release and continue with the next eligible release. HaruHaru and Solwave Radio are not fallbacks: they deliberately use app still-image rendering from the uploaded cover.
- Keep these assets separate for normal moving-video channels: `--thumbnail` is the click image, `--cover` is the playback visual/first frame, and `--loop-video` is the short moving visual used inside the rendered video. Do not put channel names or logos on any of them. For HaruHaru, upload `--cover` and `--thumbnail` only, then render with `--allow-still-image-video --video-render-source-mode still_image`; do not upload `--loop-video` unless the human explicitly asks for motion. HaruHaru cover/thumbnail prompts should explicitly avoid professional photoshoot language and tight frontal portrait language; use friend-taken travel/cafe snapshot language instead. For Solwave Radio, upload `--cover` and `--thumbnail` only, then render with `--allow-still-image-video --video-render-source-mode still_image --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`; do not upload `--loop-video` unless the human explicitly asks for motion. Solwave cover/thumbnail prompts should explicitly avoid professional photographer, studio portrait, fashion-campaign, and AI-beauty headshot language; use friend-taken night-out/travel/cafe phone snapshot language instead. For `불송`, use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase when useful.
...
- A final 16:9 cover image is required. For moving-video releases, this cover also acts as the Gemini/Dreamina/Seedance first-frame reference. For HaruHaru and Solwave Radio, it is the still-image render source. Do not put the selected channel name, a channel logo, or a brand line on it. Use only a short style, genre, use-case, or passage phrase when visual text is useful.
- A separate YouTube thumbnail image is required for normal channels. Most channels use readable click text. For HaruHaru, create a text-free photorealistic lifestyle thumbnail derived from the cover unless the human explicitly asks for text. For Solwave Radio, create a photorealistic friend-taken phone-photo lifestyle thumbnail derived from the cover with one short integrated Latin/Spanish lane phrase when useful. For `불송`, use the same cover/first-frame package as the thumbnail, usually with the short Korean passage/theme + style phrase. For J-pop/Japan singles, use natural J-pop lane text such as `J-POP`, `CITY POP`, or `ANIME POP`, not the channel name. For Storylight OST, Cinematic Pulse, Club Bloom, BibliaCanto, and sundaze, use the selected channel profile and song concept for thumbnail wording; there is no fixed recurring channel-brand signature. Do not add duration text or badges such as `1 HOUR`, `60 MIN`, or `1時間`.
- Apply the selected channel profile to both static images. For J-pop/Japan/Tokyo Daydream Radio singles, use the Tokyo profile. For Soft Hour/default BGM singles, use the Soft Hour profile. For HaruHaru, Solwave Radio, Storylight OST, Cinematic Pulse, Club Bloom, BibliaCanto, 불송, or sundaze, use that channel's profile and let the song concept drive the scene. The thumbnail should be generated from the final cover as a reference/edit derivative with the same composition plus readable click text where the profile calls for it. HaruHaru thumbnails should normally keep the same photorealistic scene without text. Solwave Radio thumbnails should keep the same photorealistic Latin/Spanish friend-taken phone-photo scene with at most one integrated lane phrase. In thumbnails, keep the main requested subject centered; text must fit around the composition rather than moving the subject sideways.
...
- Keep the visual style animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Solwave Radio, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru and Solwave Radio use photorealistic lifestyle still images by default, with Solwave using a casual friend-taken smartphone/Instagram look; 불송 may occasionally use cute/gentle animation when the music lane is also soft and bright enough to match.
...
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail has readable click text except on HaruHaru, where it should normally be text-free. The cover/loop video use the cleaner first-frame composition. None should contain channel names. The loop video must remain free of subtitles, lyrics, UI, logos, title sentences, duration text, and unrelated words. For HaruHaru, upload only the final cover and text-free thumbnail, then render from `still_image` with lower-left lyrics and lower-right spectrum. For Solwave Radio, upload only the final cover and photorealistic friend-taken phone-photo thumbnail, then render from `still_image` with lower-left lyrics and lower-right spectrum.
...
After one generated audio file, final cover, final thumbnail, and the required visual source are ready, run one command. Moving-video channels need a short loop video. For HaruHaru and Solwave Radio, do not prepare a loop video; pass the still-image render flags instead. For `불송`, use the same calm Buddhist visual package as the thumbnail and omit `--thumbnail` or pass `--allow-cover-as-thumbnail`:
...
- Do not publish without final cover and thumbnail handling for the correct YouTube channel. This means a final cover/first-frame without channel names plus a separate thumbnail. For HaruHaru, the cover and thumbnail are photorealistic text-free lifestyle still images by default. For Solwave Radio, the cover and thumbnail are photorealistic Latin/Spanish friend-taken phone-photo still images by default, with one short integrated lane phrase on the thumbnail when useful. For `불송`, this can be one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase.
...
- Keep every generated visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Solwave Radio, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic Korean lifestyle still images by default, Solwave Radio uses photorealistic Latin/Spanish friend-taken smartphone lifestyle still images by default, and 불송 may occasionally use cute/gentle animation when the music lane is also soft and bright enough to match.
- Generate a short motion clip before running the full publish command for moving-video channels, then pass it with `--loop-video` and `--loop-video-provider gemini|dreamina|seedance`. Try Gemini first unless Gemini is on cooldown; use Dreamina/Seedance when Gemini is unavailable, has reached 3 successful videos in the last 24 hours, or cannot create a video after 10 blocked prompt attempts. If Dreamina/Seedance cannot create the clip, try Gemini again if quota is available; if Gemini quota is exhausted, defer this release and resume it first after the 24 hour Gemini cooldown. For normal HaruHaru and Solwave Radio releases, do not create a motion clip and do not pass `--loop-video`; render with `--allow-still-image-video --video-render-source-mode still_image`.
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail contains readable click text except on HaruHaru, where it should normally be text-free; the cover and loop video use the cleaner first-frame composition. None should contain channel names. For HaruHaru and Solwave Radio, verify the cover/thumbnail leave lower-left and lower-right space for app-managed lyrics and spectrum. For `불송`, use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase when useful. Verify that Gemini/Dreamina/Seedance preserves the intended frame in moving clips.
- A Gemini/Veo provider logo or watermark in the corner is allowed and is not a reason to regenerate. Do not add any other logos or UI yourself.
...
- The release needs a final cover/first-frame without channel names, a separate thumbnail, visual render handling, metadata approval, and app-managed upload. Moving-video channels need a short Gemini-first loop video. HaruHaru and Solwave Radio need `still_image` render flags instead of a loop video. Exception: `불송` can use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase.
