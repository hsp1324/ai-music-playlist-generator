...
- Temporary Suno credit freeze: from 2026-06-05 through 2026-06-12, and until the human confirms credits are restored on or after 2026-06-13, do not open Suno, press Create, spend credits, generate new audio, retry short generations, extend songs, or create remixes. During this freeze, use existing approved tracks through `search-tracks` / `reuse-track` for non-scripture playlist work, while still creating visual assets and queuing audio/video render. Existing audio already generated/downloaded before the freeze can still be uploaded because the credit was already spent.
...
- Soft Hour Radio is piano-first by default now. When Suno generation is allowed, generate roughly the first 15 minutes as new solo piano / felt piano / quiet piano tracks, then let the app reuse previous same-channel similar tracks for the remaining duration. During the credit freeze, choose existing approved Soft Hour piano-first/similar tracks instead of generating. The app prefers previous Soft Hour solo-piano tracks first; if there are not enough piano tracks to approach one hour, it may fill the remaining back half with similar existing Soft Hour music instead of blocking. Do not generate new lofi beats, guitar, Rhodes, strings, pads, drums, percussion, jazz trio, bossa, or other mixed-instrument BGM for Soft Hour unless the human explicitly changes this rule.
...
- Cinematic Pulse is no longer a standing still-image exception. For normal Cinematic Pulse releases, create photorealistic 2560x1440 or 1920x1080 cover/first-frame art with one tasteful upper-left cinematic style phrase such as `MOVIE OST`, `CINEMATIC ORCHESTRA`, `FILM SCORE`, `TRAILER MUSIC`, `DARK FANTASY`, or `HEROIC MUSIC`; never use the channel name. Generate a subtle 7 second provider loop video from that text-integrated first frame, upload it with `--loop-video`, and queue `scripts/openclaw-release render-video --release-id RELEASE_ID --video-render-source-mode loop_video --video-render-resolution 720p --video-spectrum-overlay-style bars`. Use `--allow-still-image-video --video-render-source-mode still_image` only when a human explicitly asks for a still-image fallback.
...
- Current Suno credit-freeze rule: from 2026-06-05 through 2026-06-12, and until the human confirms credits are restored on or after 2026-06-13, do not open Suno, generate new Suno audio, retry short generations, extend songs, or create remixes. Existing audio that was already generated/downloaded before this rule was noticed can still be uploaded and used because the credit has already been spent. During the freeze, build non-scripture releases from existing approved similar tracks with `scripts/openclaw-release search-tracks` and `scripts/openclaw-release reuse-track`; keep one coherent style lane, prefer liked tracks, and exclude disliked/copyright-blocked/reuse-disabled tracks. Do not start fresh `BibliaCanto` or `불송` passage-based releases that need new passage-specific audio; only finish those releases when the matching generated/downloaded audio already exists.
- Create roughly `900` seconds / 15 minutes of new approved audio for non-scripture channels except `Storylight OST`. If Suno is unstable, stop generation, upload only the tracks already created, and call audio render instead of burning credits on retries. For Soft Hour Radio, this new lead block must be solo piano only and uploaded first. The app will try to extend the base block to roughly `3600` seconds / 60 minutes by reusing tracks from the back half of previous same-channel, similar-genre YouTube uploads; for Soft Hour, it prefers similar solo-piano tracks first and then falls back to similar existing Soft Hour music if piano is not enough. Within eligible similar candidates, liked tracks are selected before neutral tracks, disliked/copyright-blocked/reuse-disabled tracks are never reused, and then least-used/recent back-half material is preferred. If similar reuse material is not available, a release over `2400` seconds / 40 minutes is still publishable.
...
```text
You are creating and publishing a Playlist Release through the AI Music app. Current temporary Suno credit freeze: from 2026-06-05 through 2026-06-12, and until the human confirms credits are restored on or after 2026-06-13, do not open Suno or generate new audio. During that freeze, build non-scripture releases by recombining existing approved same-channel, similar-style tracks with `search-tracks` and `reuse-track`; still make/upload cover, thumbnail, and required visual assets, then render. Existing already-generated/downloaded audio may still be uploaded because the credit was already spent. Outside the freeze, for non-scripture channels except Storylight OST, create roughly 15 minutes of new audio; the app will try to extend the base block to about 60 minutes through same-channel similar-genre reuse, while allowing publish once it is over 40 minutes if reuse is limited. For Soft Hour Radio, that new 15-minute lead block must be solo piano only; the reused back half is piano-first, then similar existing Soft Hour music if piano is not enough. Storylight OST is reuse-only by default: build it from existing approved Storylight-compatible tracks instead of making new Suno audio. During the trial period, final video repeat is disabled.
...
- While the Suno credit freeze is active, never open Suno or generate new audio. Search and attach existing approved similar tracks instead. If the selected channel/lane cannot be built honestly from existing tracks, choose another eligible non-scripture lane; for `BibliaCanto` or `불송`, only finish releases whose matching passage/Buddhist audio already exists locally.
