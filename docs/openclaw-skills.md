...
- Playlist track generation should target roughly 3:00 to 3:30 per Suno output, with 3:45 still in the preferred range. Do not target short 2:20-2:45 radio-edit pop songs for one-hour playlists. Outputs up to 4:45 are acceptable if Suno returns them with a complete ending, but do not intentionally ask for long tracks. Regenerate or replace outputs over 4:45 before publishing. `auto-publish-playlist` rejects tracks under 180 seconds unless `--allow-short-track` is used with explicit human approval, and rejects tracks over 285 seconds unless `--allow-long-track` is used with explicit human approval.
- Do not ask Suno to fade out, slow fade, dissolve, trail away, end by lowering volume, or use an unresolved outro. This is not the desired channel style. A slightly longer complete song is always better than a track that fades out mid-idea. Prompts, style strings, lyrics, and bracketed metatags should explicitly say `no fade-out`, `complete ending`, `final cadence`, `resolved ending`, `hard ending`, `clear final chord`, or `final hit` where appropriate.
- Before uploading every playlist track, listen to or inspect the final 20-30 seconds. Reject/regenerate any track where the song keeps playing while volume gradually drops, where a chorus/loop is faded out to satisfy time length, or where the ending sounds cut off. The helper does not automatically reject fade-out endings because ambience, crowd, applause, or cinematic tail endings can cause false positives; this is a generation/retry quality decision.
- For J-pop/K-pop/English pop/Latin pop/Spanish pop vocal tracks, prompt Suno for a complete song form that naturally lasts 3:00-3:30: intro, verse, pre-chorus, chorus, second verse, bridge or instrumental break, final chorus, and a resolved outro with a clear final chord/hit. If Suno returns a 2:30-ish track or a track that fades out mid-idea, extend/regenerate it instead of treating it as a normal playlist track unless the human explicitly accepts it.
...
- Every YouTube description, including `ko`, `ja`, `en`, `es`, `vi`, `th`, `hi`, `fil`, `id`, `pt-BR`, `fr`, `de`, `ar`, `zh-CN`, and `zh-TW` localized descriptions, must end with a visible public hashtag line. The `--tags` option is separate API metadata and does not create visible hashtags in the description.
...
- Cinematic mystery/storytelling BGM, documentary BGM, investigation music, noir, dark ambient, urban legend, thriller writing music, or similar instrumental story-background releases go to `Midnight Cue Radio`. If `/youtube/status` still shows `AI썰전`, treat it as legacy Midnight Cue Radio until the human manually renames/reconnects it. This channel is instrumental/no-vocal in the current music automation pipeline, not a talk/debate channel.
...
- YouTube metadata supports localized title/description for `ko`, `ja`, `en`, `es`, `vi`, `th`, `hi`, `fil`, `id`, `pt-BR`, `fr`, `de`, `ar`, `zh-CN`, and `zh-TW`. For `Tokyo Daydream Radio`, `HaruHaru`, `Midnight Cue Radio`, `sundaze`, `Solwave Radio`, or any pop-family/story-BGM release, OpenClaw must write all fifteen versions. Use Korean as the default for Tokyo/Soft Hour/HaruHaru unless the channel profile says otherwise. Use `--default-language en` for `sundaze` and `Midnight Cue Radio`; use `--default-language es` for `Solwave Radio`. Always pass `--ko-title`, `--ko-description-file`, `--ja-title`, `--ja-description-file`, `--en-title`, `--en-description-file`, `--es-title`, `--es-description-file`, `--vi-title`, `--vi-description-file`, `--th-title`, `--th-description-file`, `--hi-title`, `--hi-description-file`, `--fil-title`, `--fil-description-file`, `--id-title`, `--id-description-file`, `--pt-title`, `--pt-description-file`, `--fr-title`, `--fr-description-file`, `--de-title`, `--de-description-file`, `--ar-title`, `--ar-description-file`, `--zh-title`, `--zh-description-file`, `--zh-tw-title`, and `--zh-tw-description-file` to `scripts/openclaw-release approve-metadata`.
...
- `HaruHaru` title example: `[playlist] 비 오는 서울 K-POP 믹스 | 늦은 밤 버스, 이별, 감성 보컬`.
- `Midnight Cue Radio` title example: `[playlist] Midnight Mystery Documentary BGM | Dark Cases, Archives and Investigation Music`.
...
- HaruHaru, Midnight Cue Radio, sundaze, and Solwave Radio do not have fixed visual signatures yet. Use the selected channel profile and let the playlist concept drive the cover, thumbnail, and 10 second loop video.
...
- Dreamina/Seedance may silently keep or return to the `5 seconds` default. For normal OpenClaw-created releases, before clicking generate, explicitly confirm the duration control says `10 seconds`. After download, check the MP4 duration. If it is around 5 seconds and the human did not explicitly ask to use a 5 second clip, discard it, return to Dreamina, set duration to `10 seconds`, regenerate, and only then upload/pass `--loop-video`.
...
For mainstream J-pop/Japanese pop, Tokyo/Japan pop, city pop, dance-pop, synth-pop, pop-rock, anime-pop, or similar Japan-themed vocal pop singles, publish to `Tokyo Daydream Radio`. For Korean/K-pop singles, publish to `HaruHaru`. For cinematic mystery/storytelling BGM singles, publish to `Midnight Cue Radio` unless the human says it is a normal calm BGM single. For English/American pop singles, publish to `sundaze`. For Latin/Spanish pop singles, publish to `Solwave Radio`.
...
- A separate YouTube thumbnail image with readable text is required. For J-pop/Japan singles, use the approved Tokyo Daydream Radio pattern: large `J-POP` with smaller `TOKYO DAYDREAM RADIO` beneath it. For HaruHaru, Midnight Cue Radio, sundaze, and Solwave Radio, use the selected channel profile and song concept for thumbnail wording; there is no fixed recurring visual signature yet. Do not add duration text or badges such as `1 HOUR`, `60 MIN`, or `1時間`.
- Apply the selected channel profile to both static images. For J-pop/Japan/Tokyo Daydream Radio singles, use the Tokyo profile. For Soft Hour/default BGM singles, use the Soft Hour profile. For HaruHaru, Midnight Cue Radio, sundaze, or Solwave Radio, use that channel's profile and let the song concept drive the scene. The cover should contain only the large lower-left channel brand label; the thumbnail must be generated from that final cover as a reference/edit derivative, using the same composition plus readable click text and channel branding. In the thumbnail, keep the main channel/requested subject centered; text must fit around the composition rather than moving the subject sideways.
...
- If using browser automation, open `https://dreamina.capcut.com/ai-tool/home/`, select `2.0 Fast`, use first/last-frame mode with only the first frame provided, do not use Omni Reference, leave the last frame empty, set `16:9`, `720p`, and `10 seconds` when selectable, create/download the MP4, confirm the local file exists, and pass that absolute path as `--loop-video`.
...
- Publish Japanese/J-pop/Tokyo content to `Tokyo Daydream Radio`, Korean/K-pop content to `HaruHaru`, cinematic mystery/storytelling BGM to `Midnight Cue Radio`, English/American pop to `sundaze`, and Latin/Spanish pop to `Solwave Radio`.
...
Use `Soft Hour Radio` for normal background/cafe/sleep/study/chill releases. Use `Tokyo Daydream Radio` for mainstream J-pop/Japanese pop, Tokyo/Japan pop, city pop, dance-pop, synth-pop, pop-rock, anime-pop, or similar Japan-themed vocal pop releases. Use `HaruHaru` for Korean/K-pop vocal releases. Use `Midnight Cue Radio` for cinematic mystery/storytelling BGM releases; if the connected title is still `AI썰전`, treat it as legacy Midnight Cue Radio. Use `sundaze` for English/American pop. Use `Solwave Radio` for Latin/Spanish pop. If the next-release planner selected another connected non-excluded channel, pass that channel title explicitly and use the returned custom or dedicated channel docs.
...
- Ask Suno for roughly 3:00-3:30 per playlist track. 3:45 is still fine. For vocal pop-family music, avoid 2:30-ish radio-edit outputs; ask for a full arrangement with intro, verse, pre-chorus, chorus, second verse, bridge or instrumental break, final chorus, and a resolved outro with a clear final chord/hit. Do not request fade-outs, slow fades, or volume-lowering endings. Tracks up to 4:45 are acceptable if Suno returns them with a complete ending, so a 4:04 Suno result can be used. If Suno produces a track under 3:00, extend/regenerate it or pass `--allow-short-track` only with explicit human approval. If Suno produces a fade-out/cut-off ending, regenerate/extend it even if the duration is otherwise good. If Suno produces a track longer than 4:45, regenerate or replace it instead of uploading it for final publish unless the human explicitly approves the longer track.
...
- Publish privately to the selected YouTube channel. Use `Tokyo Daydream Radio` for mainstream J-pop/Japanese pop/Tokyo pop releases, `HaruHaru` for Korean/K-pop vocal releases, `Midnight Cue Radio` for cinematic mystery/storytelling BGM releases, `sundaze` for English/American pop, `Solwave Radio` for Latin/Spanish pop, and `Soft Hour Radio` for default BGM/background releases. If the planner selected another connected non-excluded channel, use that exact channel title and the returned channel docs.
- Return the command output JSON, including release.id, uploaded track ids, YouTube video id, and output paths.
...
If the release is mainstream J-pop/Japanese pop/Tokyo pop, set `--youtube-channel-title "Tokyo Daydream Radio"`. If it is Korean/K-pop, set `--youtube-channel-title "HaruHaru"`. If it is cinematic mystery/storytelling BGM, set `--youtube-channel-title "Midnight Cue Radio"`; the helper can resolve legacy connected title `AI썰전`. If it is English/American pop, set `--youtube-channel-title "sundaze"`. If it is Latin/Spanish pop, set `--youtube-channel-title "Solwave Radio"`. Otherwise set `--youtube-channel-title "Soft Hour Radio"` or omit the flag and let the helper infer the default.
...
- Do not upload public. The final upload must be private.
- Do not publish if the selected YouTube channel is not connected. Current intended routing is `Soft Hour Radio` for general BGM releases, `Tokyo Daydream Radio` for mainstream J-pop/Japanese pop releases, `HaruHaru` for Korean/K-pop vocal releases, `Midnight Cue Radio` for cinematic mystery/storytelling BGM releases, `sundaze` for English/American pop, and `Solwave Radio` for Latin/Spanish pop.
...
- Do not accidentally use a 5 second Dreamina default clip for normal auto-publish work. `scripts/openclaw-release` allows alternate clip lengths when intentionally provided, so OpenClaw must catch the Dreamina UI mistake before upload by checking the generated file duration and regenerating at 10 seconds unless the human explicitly accepts the shorter clip.
