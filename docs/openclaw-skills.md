...
- Preferred thumbnail format: full-bleed image background, no card or panel, large bottom-left genre/mood text, and a smaller channel-brand line directly below. For Japan/J-pop releases, use the approved `J-POP` + `TOKYO DAYDREAM RADIO` treatment.
...
- Dreamina prompt shape: `Use the uploaded first-frame image as the exact starting frame. Create one continuous ambient Tokyo night J-pop synth-pop video shot. The motion must progress forward naturally for the full clip. Do not repeat any segment. Do not ping-pong or restart motion. Preserve the opening composition, lighting, palette, and baked-in text only at the start. Add subtle slow camera drift, neon reflections, rain shimmer, and soft city light motion. Stable composition, no hard cuts, no new text, no subtitles, no logos, no UI, no new people or characters.`
...
- Use the approved/final thumbnail as the visual starting reference for Dreamina/Seedance image-to-video generation. The first shot of the 6 second loop video should look like the thumbnail, but the loop video itself should not add extra text, subtitles, lyrics, logos, or UI elements beyond what is already baked into the thumbnail reference.
- For Dreamina motion clips, set duration/ratio/quality in Dreamina controls, not in the prompt. The prompt should request `one continuous forward-moving video shot`, `smooth gentle motion`, `slow camera motion`, `stable composition`, `no repeated segment`, `no extra text overlays`, `no subtitles`, `no logos`, and `no hard cuts`. Do not ask for a matching last frame; the app handles smooth repeat with forward crossfade rendering.
