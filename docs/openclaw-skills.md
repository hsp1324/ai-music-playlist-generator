...
- HaruHaru, Storylight OST, Cinematic Pulse, Club Bloom, The Old Verse, The New Verse, sundaze, and Solwave Radio do not have fixed visual signatures yet. Use the selected channel profile and let the playlist concept drive the cover, thumbnail, and short loop video. Cinematic Pulse normally skips the loop video and renders from its high-resolution still cover.
...
- Do not rely on the default app-rendered visualizer preset. For each final render, choose the visualizer preset that best fits the cover/loop video, channel, and composition, then pass it explicitly with `--video-spectrum-overlay-style bars|multiwave|mirror-bars|radial|pulse|none`. Use `bars` or `mirror-bars` for clean pop/EDM energy, `pulse` for punchy dance/pop, and avoid `multiwave` unless the design still looks clean. For very calm BGM where the overlay would distract, pass `none`. For Cinematic Pulse, always pass `--video-spectrum-overlay-style bars` unless the human explicitly asks otherwise; do not use radial, pulse, multiwave, dots/particles, or busy waveform styles there. Do not use the removed small-dot visualizer style, and do not use the thin waveform style used on `아침 온실 피아노 BGM`; both looked visually busy. Legacy `dot` / `dots` / `particles` / `thinwave` / `thin-wave` / `clean-wave` values fall back to `bars`.
