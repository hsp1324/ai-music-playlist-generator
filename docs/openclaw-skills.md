...
- For scripture releases, read [openclaw-scripture-sequence.md](openclaw-scripture-sequence.md), create the release with upload channel `The Old Verse`, reserve the next app-owned canonical passage with `scripts/openclaw-release openclaw-scripture-reserve` before Suno generation, include the returned `entry.passage_range` in the main and localized YouTube titles, and mark it scheduled/published with `scripts/openclaw-release openclaw-scripture-complete` after successful upload/scheduling. Reserve Old Testament with `--channel-title "The Old Verse"` and New Testament with `--channel-title "New Testament"`, but publish both branches with `--youtube-channel-title "The Old Verse"`. Do not compare against a local scripture ledger, and never duplicate or skip a passage unless the human explicitly says so and the app ledger is updated accordingly.
...
- YouTube visibility is app-managed. With `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads as a scheduled public release at the next free slot for the selected YouTube channel; schedules on other channels do not block that channel. Scripture releases on `The Old Verse` have two daily slots in the schedule timezone: Old Testament at 07:00 and New Testament at 16:00. Otherwise it falls back to `AIMP_YOUTUBE_PRIVACY_STATUS`. Do not manually change visibility from OpenClaw.
- For `The New Verse` Buddhist releases, publish through the app normally but keep them private for now. The app skips scheduled-public slots for this channel until the human reviews the new direction tomorrow.
...
- For `The Old Verse`, do not make the thumbnail only a broad label such as `GOSPEL SONGS`, `BIBLE MUSIC`, or `SCRIPTURE SONGS` plus the channel name when the release has a known passage. Include a short passage/book/theme/prayer-worship cue when it improves clarity, while keeping the wording natural and not overfitted to one repeated template. For `The New Verse`, make Buddhist source/theme and lane clear, such as `법구경 힙합`, `반야심경 R&B`, `BUDDHIST JAZZ`, or `DHARMA R&B`.
...
- All static images and Dreamina/Seedance/Gemini loop clips must look animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. Cinematic Pulse is the explicit exception: use original photorealistic cinematic film-still / premium movie-poster realism for cover and thumbnail base, and normally render from that still cover instead of a provider loop video.
...
- Keep the visual style animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. Cinematic Pulse is the explicit exception and should use original photorealistic cinematic film-still / movie-poster realism.
...
- Publish Japanese/J-pop/Tokyo vocal-pop content to `Tokyo Daydream Radio`, Korean/K-pop content to `HaruHaru`, playful no-vocal Japanese-style game/anime OST or arcade/fantasy-game BGM to `Storylight OST`, no-vocal large-scale cinematic orchestra/movie-OST/film-score BGM to `Cinematic Pulse`, no-vocal EDM/house/techno/trance club music to `Club Bloom`, Old Testament and New Testament Bible scripture music to `The Old Verse`, Buddhist scripture-inspired vocal music to `The New Verse`, English/American pop to `sundaze`, and Latin/Spanish pop to `Solwave Radio`.
...
For New Testament scripture releases, `SELECTED_CHANNEL_TITLE` is still `The Old Verse`.
