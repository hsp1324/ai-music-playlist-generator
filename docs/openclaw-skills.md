...
- Every YouTube description, including `ko`, `ja`, `en`, `es`, `vi`, `th`, `hi`, `zh-CN`, and `zh-TW` localized descriptions, must end with a visible public hashtag line. The `--tags` option is separate API metadata and does not create visible hashtags in the description.
...
- YouTube metadata supports localized title/description for `ko`, `ja`, `en`, `es`, `vi`, `th`, `hi`, `zh-CN`, and `zh-TW`. For `Tokyo Daydream Radio`, `sundaze`, `Solwave Radio`, or any pop-family release, OpenClaw must write all nine versions. Use Korean as the default for Tokyo/Soft Hour unless the channel profile says otherwise. Use `--default-language en` for `sundaze` and `--default-language es` for `Solwave Radio`. Always pass `--ko-title`, `--ko-description-file`, `--ja-title`, `--ja-description-file`, `--en-title`, `--en-description-file`, `--es-title`, `--es-description-file`, `--vi-title`, `--vi-description-file`, `--th-title`, `--th-description-file`, `--hi-title`, `--hi-description-file`, `--zh-title`, `--zh-description-file`, `--zh-tw-title`, and `--zh-tw-description-file` to `scripts/openclaw-release approve-metadata`.
...
- The app does not reject low-motion loop videos. It only checks that the upload is a readable video and close to the expected 10 second duration. OpenClaw should still visually inspect the loop and regenerate if the motion is too static for the release, but this is a quality decision rather than an app upload blocker.
...
- Dreamina/Seedance may silently keep or return to the `5 seconds` default. Before clicking generate, explicitly confirm the duration control says `10 seconds`. After download, verify the MP4 duration is close to 10 seconds before upload. If it is around 5 seconds, discard it, return to Dreamina, set duration to `10 seconds`, regenerate, and only then upload/pass `--loop-video`.
...
- Before uploading the downloaded MP4, verify it is close to 10 seconds. The helper and app accept roughly 8-12 seconds, but a 5 second Dreamina default is invalid. If the downloaded file is about 5 seconds, do not upload it and do not call publish; regenerate with the duration control set to `10 seconds`.
...
- Do not upload a 5 second Dreamina default clip. If the local `--loop-video` file is shorter than the accepted 8-12 second range, `scripts/openclaw-release` rejects it before upload; regenerate a 10 second clip instead.
