...
- Do not add subtitles, lyric overlays, UI elements, title text, or unrelated words inside the loop video. The default visual action is three people walking forward away from the viewer, but an explicit human visual/video request overrides that default. If the first-frame reference has baked-in text other than an explicitly requested lower-left channel label, regenerate a clean/no-extra-text first frame before using Dreamina/Seedance.
