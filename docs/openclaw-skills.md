...
- `cover`: video visual shown during playback and, for moving-video channels, used as the Dreamina/Seedance/Gemini first-frame reference. It should look good for the full video duration and must not include a channel name or logo. If visual text is useful, use a short style, genre, use-case, or passage phrase. For HaruHaru, this is the still-image video source.
- `thumbnail`: YouTube click thumbnail. It should usually include short readable text such as `CAFE PIANO`, `DEEP SLEEP`, `FOCUS MUSIC`, `TOKYO NIGHT`, `CITY POP`, or `J-POP`. Do not add the channel name or duration text such as `1 HOUR`, `60 MIN`, `1時間`, or time badges. Exception: HaruHaru thumbnails should normally have no text and look like natural photorealistic lifestyle photos.
...
- Storylight OST, Cinematic Pulse, Club Bloom, BibliaCanto, 불송, sundaze, and Solwave Radio do not have fixed visual signatures yet. Use the selected channel profile and let the playlist concept drive the cover, thumbnail, and short loop video. HaruHaru is different: it defaults to photorealistic Korean lifestyle still-image cover/thumbnail packaging and renders from the still cover, with no provider loop video unless the human explicitly asks for motion. Cinematic Pulse uses photorealistic cinematic first-frame art and now also uses a subtle provider loop video by default.
...
- All static images and Dreamina/Seedance/Gemini loop clips must look animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic still-image lifestyle visuals by default, 불송 also allows occasional cute/gentle animation when the release music lane fits it, and Cinematic Pulse should use restrained photorealistic cinematic motion in the provider loop video rather than animated/anime styling.
...
- Do not substitute a locally synthesized motion loop, app-rendered still-image animation, Ken Burns pan/zoom, or other non-provider workaround when Dreamina/Seedance fails. Dreamina/Seedance failure should route to Gemini if quota is available; if Gemini quota is exhausted, defer that release and continue with the next eligible release. HaruHaru is not a fallback: it deliberately uses app still-image rendering from the uploaded cover.
- Keep these assets separate for normal moving-video channels: `--thumbnail` is the click image, `--cover` is the playback visual/first frame, and `--loop-video` is the short moving visual used inside the rendered video. Do not put channel names or logos on any of them. For HaruHaru, upload `--cover` and `--thumbnail` only, then render with `--allow-still-image-video --video-render-source-mode still_image`; do not upload `--loop-video` unless the human explicitly asks for motion. For `불송`, use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase when useful.
...
- Keep the visual style animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic lifestyle still images by default, and 불송 may occasionally use cute/gentle animation when the music lane is also soft and bright enough to match.
...
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail has readable click text except on HaruHaru, where it should normally be text-free. The cover/loop video use the cleaner first-frame composition. None should contain channel names. The loop video must remain free of subtitles, lyrics, UI, logos, title sentences, duration text, and unrelated words. For HaruHaru, upload only the final cover and text-free thumbnail, then render from `still_image`.
...
After one generated audio file, final cover, text thumbnail, and short loop video are ready, run one command. For HaruHaru, do not prepare a loop video; pass the still-image render flags instead. For `불송`, use the same calm Buddhist visual package as the thumbnail and omit `--thumbnail` or pass `--allow-cover-as-thumbnail`:
...
`--loop-video` is required for normal moving-video publish automation. If it is missing, stop and create/download the Gemini/Dreamina/Seedance clip first. Do not create a local motion-loop workaround. For HaruHaru, omit `--loop-video` and pass `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. Use `--allow-still-image-video` on other channels only when the human explicitly accepts a static-cover fallback.
...
- Do not publish without final cover and thumbnail handling for the correct YouTube channel. This means a final cover/first-frame without channel names plus a separate thumbnail. For HaruHaru, the cover and thumbnail are photorealistic text-free lifestyle still images by default. For `불송`, this can be one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase.
...
- Keep every generated visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Cinematic Pulse, and 불송 are explicit photorealistic/premium cinematic-real exceptions; HaruHaru uses photorealistic Korean lifestyle still images by default, and 불송 may occasionally use cute/gentle animation when the music lane is also soft and bright enough to match.
- Generate a short motion clip before running the full publish command for moving-video channels, then pass it with `--loop-video` and `--loop-video-provider gemini|dreamina|seedance`. Try Gemini first unless Gemini is on cooldown; use Dreamina/Seedance when Gemini is unavailable, has reached 3 successful videos in the last 24 hours, or cannot create a video after 10 blocked prompt attempts. If Dreamina/Seedance cannot create the clip, try Gemini again if quota is available; if Gemini quota is exhausted, defer this release and resume it first after the 24 hour Gemini cooldown. For normal HaruHaru releases, do not create a motion clip and do not pass `--loop-video`; render with `--allow-still-image-video --video-render-source-mode still_image`.
- The thumbnail, cover, and loop video are separate assets for moving-video channels. The thumbnail contains readable click text except on HaruHaru, where it should normally be text-free; the cover and loop video use the cleaner first-frame composition. None should contain channel names. For HaruHaru, verify the cover/thumbnail leave lower-left and lower-right space for app-managed lyrics and spectrum. For `불송`, use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase when useful. Verify that Gemini/Dreamina/Seedance preserves the intended frame in moving clips.
- A Gemini/Veo provider logo or watermark in the corner is allowed and is not a reason to regenerate. Do not add any other logos or UI yourself.
...
- If the first-frame image itself appears to be blocked on a moving-video channel, regenerate a safer first-frame/cover image and then retry. If Dreamina/Seedance still cannot create the clip, try Gemini if quota is available. If Gemini quota is exhausted, defer this release, report the deferral in Slack, and continue with the next eligible release before render/publish unless the human explicitly accepts a still-image fallback. If accepted, pass `--allow-still-image-video`. HaruHaru already uses still-image render by default, so this failure path does not apply unless the human explicitly requested motion.
- If Dreamina login, CAPTCHA, payment, face detection, or human approval blocks browser automation, do not create a local motion-loop substitute. Try Gemini if quota is available; if Gemini quota is exhausted, defer this release and move on. For HaruHaru, avoid this path by not creating a provider loop video in normal automation.
- For HaruHaru photorealistic releases, prompt for clearly adult stylish subjects with visible natural faces allowed. Do not hide faces by default, and reject/regenerate only for explicit, minor-coded, celebrity-like, policy-blocked, badly distorted, or visually unusable results.
...
For HaruHaru, omit `upload-loop-video` and queue the still-image render instead:
...
Do not omit `--cover` or `--thumbnail` for normal playlist production. Do not omit `--loop-video` for moving-video channels. For HaruHaru, intentionally omit `--loop-video` and render with `still_image`. If any required asset is not ready, stop and report the missing asset. The app's local draft cover is only a placeholder for manual review, not acceptable for automatic YouTube upload.
...
- Do not publish if final YouTube thumbnail handling is missing. For normal channels, final YouTube thumbnail art must be uploaded. For HaruHaru, this thumbnail should normally be a text-free photorealistic lifestyle derivative of the cover. For `불송`, reuse the same cover/first-frame package as the thumbnail, with the short Korean passage/theme + style phrase when present.
- Do not use Dreamina or Gemini to create static cover or thumbnail images. Use OpenAI GPT Image models for static images, then use Gemini-first video generation to animate the cover or first-frame image into a short loop video for moving-video channels. HaruHaru skips provider video generation and renders from the still cover. Do not put channel names, channel logos, or channel-brand lines on any generated visual.
...
- Do not create a long MP4 in OpenClaw. For moving-video channels, upload only the short loop clip with `--loop-video` and a provider tag via `--loop-video-provider`; the app handles the long render. For HaruHaru, do not upload a loop clip and let the app render from `still_image`.
- Do not add subtitles, lyric overlays, UI elements, title sentences, duration text, channel names, logos, or unrelated words inside the loop video. The selected channel profile controls the default visual action. An explicit human visual/video request overrides that channel default. If the first-frame reference has too much text, regenerate a cleaner first frame before using Dreamina/Seedance. For HaruHaru, keep lyrics and spectrum out of the static images; the app adds them during still-image render.
...
- The release needs a final cover/first-frame without channel names, a separate thumbnail, visual render handling, metadata approval, and app-managed upload. Moving-video channels need a short Gemini-first loop video. HaruHaru needs `still_image` render flags instead of a loop video. Exception: `불송` can use one cover/thumbnail/first-frame package with a short Korean passage/theme + style phrase.
