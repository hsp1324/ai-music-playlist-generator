...
Choose the next channel and a fresh playlist concept that fits that channel, avoids recent repetition, and can be pushed into the backlog queue safely. Suno credits are available again after the human added 500 credits on 2026-06-05, so OpenClaw may select concepts that need new Suno generation. Spend credits conservatively: normal playlist channels and 불송 normally need about 10 minutes of new audio, then app-side same-channel/similar-genre reuse fills the rest toward one hour or longer. `Storylight OST` remains reuse-only by default unless the human explicitly asks for new Storylight music. BibliaCanto should aim for about 60 minutes of new passage-based audio and is not backfilled from old Bible passages.
...
- For `불송` Buddhist releases: selected Buddhist source/theme, exact source/chapter/section when verified, release-level music lane, title hook plan that uses the source/theme instead of generic `불경` alone and avoids redundant `한국어 랩` / `한국어 힙합`, `schedule=daily_07:00_Asia/Seoul`, a fresh `visual_signature` from the 불송 profile such as `adult_temple_stay_woman_listener`, `hongdae_streetwear_buddhist_hiphop_woman`, `buddha_statue_plus_modern_woman_listener`, `two_women_duet_in_buddhist_setting`, or `female_coded_stylized_practitioner`, `thumbnail=same_cover_first_frame_package`, and `spectrum=calm-bars`
