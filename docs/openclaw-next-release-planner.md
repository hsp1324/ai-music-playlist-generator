...
Choose the next channel and a fresh playlist concept that fits that channel, avoids recent repetition, and can be pushed into the backlog queue safely. Current temporary Suno credit freeze: from 2026-06-05 through 2026-06-12, and until the human confirms credits are restored on or after 2026-06-13, do not select concepts that require new Suno generations. During the freeze, pick non-scripture channels/lanes that can be built honestly by recombining existing approved same-channel, similar-style tracks. If no suitable reusable tracks exist for one lane, choose another eligible lane/channel instead of opening Suno. Outside the freeze, non-scripture channels normally need about 15 minutes of new audio, except Storylight OST, which should reuse existing approved tracks instead of generating Suno audio; BibliaCanto and 불송 still need about 40 minutes of new passage-based audio.
...
After selecting a channel, run `scripts/openclaw-release channel-profile` with the selected channel title and read [openclaw-channel-genre-taxonomy.md](openclaw-channel-genre-taxonomy.md). Pick one underused broad YouTube playlist bucket and one detailed video style lane before Suno generation, or before existing-track search while the Suno credit freeze is active. During the freeze, do not pick fresh `BibliaCanto` or `불송` passage-based concepts unless the matching generated/downloaded audio already exists and only needs upload/resume; choose another reusable non-scripture lane instead. If the selected channel is `BibliaCanto`, first read [openclaw-scripture-sequence.md](openclaw-scripture-sequence.md). Create the release, then reserve the next app-owned canonical Old Testament or New Testament passage with `scripts/openclaw-release openclaw-scripture-reserve` before generating audio. If the selected channel is `불송`, use its Buddhist concept/profile docs and do not use the Bible scripture ledger. Read both returned docs:

- `concept_doc`: choose the next playlist concept and avoid recent repetition.
- `profile_doc`: generate cover, thumbnail, and the required visual source without mixing channel visual signatures. This is a short loop video for moving-video channels, but a still-image render package for HaruHaru, sundaze, and Solwave Radio.
...
- For `불송` Buddhist releases: selected Buddhist source/theme, release-level music lane, `schedule=daily_07:00_Asia/Seoul`, `visual_signature=respectful_photorealistic_buddha_listening_to_music_while_reading_open_sutra`, `thumbnail=same_cover_first_frame_package`, and `spectrum=calm-bars`
