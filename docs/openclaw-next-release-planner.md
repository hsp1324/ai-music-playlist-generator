...
Choose the next channel and a fresh playlist concept that fits that channel, avoids recent repetition, and can be pushed into the backlog queue safely. Suno credits are available again after the human added 500 credits on 2026-06-05, so OpenClaw may select concepts that need new Suno generation. Spend credits conservatively: non-scripture channels normally need about 10 minutes of new audio, then app-side same-channel/similar-genre reuse fills the rest toward one hour or longer. `Storylight OST` remains reuse-only by default unless the human explicitly asks for new Storylight music. BibliaCanto and 불송 should aim for about 60 minutes of new passage-based audio.
