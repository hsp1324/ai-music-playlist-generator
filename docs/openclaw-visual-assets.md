...
- Available app-rendered visualizer presets are `bars`, `multiwave`, `mirror-bars`, `radial`, `pulse`, and `none`. Do not blindly use the default. Choose the preset for the release art: `bars` / `mirror-bars` for clean pop, EDM, and dance energy; `radial` for centered scripture or dramatic non-Cinematic-Pulse cover layouts; `pulse` for punchy pop/club motion. Cinematic Pulse must use `bars` unless the human explicitly asks otherwise. Do not use the removed small-dot visualizer style, and do not use the thin waveform style used on `아침 온실 피아노 BGM`; both looked visually busy. Legacy `dot` / `dots` / `particles` / `thinwave` / `thin-wave` / `clean-wave` values fall back to `bars`. Avoid `multiwave` unless it remains clean with the image. Use `none` when the human wants faster render, no audio-reactive overlay, or a very calm BGM release where even a small waveform would feel distracting.
