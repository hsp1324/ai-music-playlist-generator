...
- Available app-rendered visualizer presets are `bars`, `multiwave`, `thinwave`, `mirror-bars`, `radial`, `pulse`, and `none`. Do not blindly use the default. Choose the preset for the release art: `bars` / `mirror-bars` for clean pop, EDM, and dance energy; `radial` for centered scripture or dramatic non-Cinematic-Pulse cover layouts; `pulse` for punchy pop/club motion; `thinwave` for calm BGM where the overlay should stay minimal. Cinematic Pulse must use `bars` unless the human explicitly asks otherwise. Do not use the removed small-dot visualizer style; it looked visually busy and legacy `dot` / `dots` / `particles` values fall back to `bars`. Avoid `multiwave` unless it remains clean with the image. Use `none` only when the human wants faster render or no audio-reactive overlay.
...
- All generated visuals should look animated, anime, illustrated, or stylized unless a channel profile says otherwise. Cinematic Pulse is the explicit exception: use original photorealistic cinematic film-still / movie-poster realism for its cover, thumbnail base, and loop video. Do not use documentary footage, real war footage, celebrity likenesses, protected IP, or copied real media.
...
- Use photorealistic cinematic film-still / premium movie-poster realism for the cover, thumbnail base, and loop video. Avoid anime, cartoon, flat illustration, painterly fantasy art, game UI art, documentary footage, real war footage, celebrity likenesses, protected IP, and exact franchise references.
...
- Queue final render with `--video-spectrum-overlay-style bars`. Keep the spectrum as a clean restrained bar visualizer; do not use radial, pulse, multiwave, dots/particles, or busy waveform styles for Cinematic Pulse unless the human explicitly asks.
