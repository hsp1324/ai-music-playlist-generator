...
- The cover is the playback visual and Dreamina/Seedance first-frame reference. It must include only a medium-sized, readable lower-left selected-channel-name watermark. Do not make channel names tiny; target roughly 7-9% of the image width with enough contrast or subtle backing shadow/glow for mobile playback and video compression.
...
- The thumbnail should use larger click text plus a channel-brand line whose size/style stays consistent with the cover watermark when possible.
