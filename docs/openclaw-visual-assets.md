...
- If all 10 Dreamina attempts fail, send `scripts/openclaw-release slack-notify --text "영상 생성이 10회 실패해서 중단했습니다. RELEASE_TITLE: ERROR_SUMMARY"` and stop before render/publish unless the human explicitly approves a still-image fallback. If that fallback is approved, pass `--allow-still-image-video`; otherwise the app rejects video render without an uploaded 10 second loop video.
