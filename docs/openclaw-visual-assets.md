...
- Available app-rendered visualizer presets are `bars`, `multiwave`, `thinwave`, `dots`, `mirror-bars`, `radial`, `pulse`, and `none`. Use the default `bars` unless the human asks for another effect. Use `none` only when the human wants a faster render without an audio-reactive overlay.
