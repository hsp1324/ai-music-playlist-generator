...
- Cover, YouTube thumbnail, first-frame, and loop video must all use the same calm Buddhist image package for 불송. It may contain one short Korean passage/theme + style phrase, such as `팔정도 명상팝` or `자비 트립합`, but never the `불송` channel label.
