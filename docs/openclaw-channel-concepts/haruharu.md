...
- Keep visual locations and props out of lyric-writing by default. Do not use `홍대`, `Hongdae`, `커튼`, `curtain`, `네오`, `neo`, `네온`, `네온사인`, `neon`, `neon sign`, or `neon signs` in HaruHaru lyrics, including compounds, translations, alternate spacing, or casing. Do not replace them with close scene-setting variants. More broadly, do not lean on arbitrary neighborhood, room-decor, signage, or prop nouns as repeated lyric hooks; use natural emotional detail and song-specific imagery instead. Scan and rewrite the lyric before upload if any banned term remains. This restriction is for lyrics only—relevant terms can still be used for visual planning or metadata.
