...
- OpenClaw should use the VM-render lookahead flow: prepare audio/cover/thumbnail/8 second loop video, run `scripts/openclaw-release render-audio`, `approve-cover`, and `render-video` without `--wait`, then release the lock. The VM renders video; when rendering completes, the app asks OpenClaw to approve metadata and publish through `scripts/openclaw-release publish-release --release-id RELEASE_ID --youtube-channel-title CHANNEL_TITLE`.
...
The app has an in-process background worker and production automation renders video on the VM. Throughput is improved by overlapping roles: while the VM renders one release, OpenClaw prepares the next release's audio, artwork, thumbnail, and loop video.
