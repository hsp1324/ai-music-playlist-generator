...
- OpenClaw can publish a rendered release with approved metadata through `scripts/openclaw-release publish-release --release-id RELEASE_ID --youtube-channel-title CHANNEL_TITLE`; this is the finisher command used by backlog automation after a distributed render worker completes video render.
