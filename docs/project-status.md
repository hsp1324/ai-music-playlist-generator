...
- `bars` is the production default; `multiwave` draws layered actual audio waveform lines, `thinwave` is a cleaner thinner waveform, `dots` draws audio-reactive glowing dots, `mirror-bars` draws centered mirrored bars, `radial` draws a large circular spectrum, `pulse` draws a punchy pulse line, and `none` skips the spectrum overlay for the fastest render
- the web `Render Video` action and OpenClaw auto-publish commands can pass a per-render visualizer preset; if omitted, `bars` is used. Use `none` for long urgent renders where speed matters more than the audio-reactive visualizer.
...
- OpenClaw playlist automation can now run `scripts/openclaw-release auto-publish-playlist` to upload generated playlist tracks as approved, skip Slack per-track review spam, render audio/video, approve generated metadata, and upload through the app to the selected connected YouTube channel. OpenClaw can also run `scripts/openclaw-release auto-publish-single` for a human-approved end-to-end single upload. When `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads each video as a scheduled public release at the next free daily 07:00 slot in `AIMP_YOUTUBE_SCHEDULE_TIMEZONE` instead of requiring a manual switch to Public. General BGM releases default to `Soft Hour Radio`; mainstream J-pop/Japanese pop/Tokyo pop releases route to `Tokyo Daydream Radio`; Korean/K-pop vocal releases route to `HaruHaru`; playful no-vocal Japanese-style game/anime OST and arcade/fantasy-game BGM routes to `Storylight OST`; no-vocal large-scale cinematic orchestra, movie OST, film score, trailer, battle, emotional, mystery-tension, and game-focus music routes to `Cinematic Pulse`; no-vocal EDM/house/techno/trance/workout/night-drive music routes to `Club Bloom`; Old Testament scripture-inspired music routes to `The Old Verse`; New Testament/Gospel/worship music routes to `The New Verse`; English/American pop routes to `sundaze`; Latin/Spanish pop routes to `Solwave Radio`. Signal Room/Signal Desk/Midnight Cue names are retired unless explicitly revived.
