...
Video render now adds an app-managed audio-reactive visualizer overlay by default:

- default style is a transparent 20-bar spectrum near the lower-right of the video
...
- `AIMP_VIDEO_SPECTRUM_OVERLAY_STYLE` can be `bars`, `multiwave`, `thinwave`, `dots`, `mirror-bars`, `radial`, or `pulse`
- `bars` is the production default; `multiwave` draws layered actual audio waveform lines, `thinwave` is a cleaner thinner waveform, `dots` draws audio-reactive glowing dots, `mirror-bars` draws centered mirrored bars, `radial` draws a large circular spectrum, and `pulse` draws a punchy pulse line
- the web `Render Video` action and OpenClaw auto-publish commands can pass a per-render visualizer preset; if omitted, `bars` is used
...
- OpenClaw playlist automation can now run `scripts/openclaw-release auto-publish-playlist` to upload generated playlist tracks as approved, skip Slack per-track review spam, render audio/video, approve generated metadata, and upload through the app to the selected connected YouTube channel. OpenClaw can also run `scripts/openclaw-release auto-publish-single` for a human-approved end-to-end single upload. When `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads each video as a scheduled public release at the next free daily 07:00 slot in `AIMP_YOUTUBE_SCHEDULE_TIMEZONE` instead of requiring a manual switch to Public. General BGM releases default to `Soft Hour Radio`; mainstream J-pop/Japanese pop/Tokyo pop releases route to `Tokyo Daydream Radio`; Korean/K-pop vocal releases route to `HaruHaru`; fantasy/fairy-tale/cozy RPG/game OST BGM routes to `Storylight OST`; cinematic/trailer/battle/game-combat BGM routes to `Cinematic Pulse`; EDM/house/dance/workout/night-drive music routes to `Club Bloom`; Old Testament scripture-inspired music routes to `The Old Verse`; New Testament/Gospel/worship music routes to `The New Verse`; English/American pop routes to `sundaze`; Latin/Spanish pop routes to `Solwave Radio`. Legacy `AI썰전` is treated as the old title for `Club Bloom` until YouTube status refreshes after the rename; Signal Room/Signal Desk/Midnight Cue names are retired unless explicitly revived.
...
- Playlist automation now keeps Suno duration wording minimal: use only `less than 4 minutes` or `under 4 minutes` when a duration hint is needed. The helper rejects playlist tracks over 4 minutes by default and no longer has a default lower-bound track duration.
...
- OpenClaw must pass a 10 second Dreamina/Seedance MP4 via `--loop-video` for normal publish automation. The video render API rejects renders without an uploaded loop video unless `allow_still_image_fallback` is explicitly set for a human-approved exception. Browser automation should use Dreamina/Seedance `2.0 Fast`, first-frame only, no Omni Reference, no last-frame reference, `16:9`, `720p`, and exactly `10 seconds` through UI controls. The first-frame image should be the cover or a separate first-frame image with only the large, readable lower-left channel label, not the YouTube text thumbnail, because large generated title text can flicker or disappear. The generated clip should end close to its opening composition so it can be reused across the full release.
- Existing releases that already have 8 second loop videos remain valid. The 10 second rule is for new OpenClaw-created loop videos going forward.
- Loop-video upload validates only that the file is readable video. The app does not reject low-motion clips or alternate clip lengths; visual quality and the normal 10 second Dreamina default are handled by human/OpenClaw review before render/publish.
...
- Dreamina/Seedance content-safety or copyright rejections are retried up to 10 total attempts with a sanitized prompt. Every failed attempt posts a Slack progress message before retrying, and 10 failures stops the automation before render/publish unless the human explicitly accepts a still-image fallback and `--allow-still-image-video` is passed.
...
- The web release detail UI now supports direct upload/replace actions for video cover, text YouTube thumbnail, and 10 second loop video as separate assets.
- Already-uploaded releases can replace the 10 second loop video from the release detail UI. Replacing the loop video marks the release as needing a new video render before re-upload while keeping the previous YouTube video id visible until the new render starts.
...
- The app can send a "make the next playlist" Slack request to the OpenClaw channel after a release is uploaded or scheduled. Configure `AIMP_OPENCLAW_SLACK_CHANNEL_ID`; the web UI shows `Request Next Playlist`, and `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_ON_PUBLISH=true` sends the request automatically after successful YouTube upload with per-video dedupe. `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_MAX_UPLOADS=N` caps the loop after N successful YouTube uploads; `0` means unlimited. Human Slack messages in the configured OpenClaw channel such as `OpenClaw 자동화 멈춰` stop the app from sending more automatic `OPENCLAW_RUN:` requests, and `OpenClaw 자동화 다시 시작` resumes. Real app-originated OpenClaw task messages are prefixed with `AIMP_OPENCLAW_SLACK_TRIGGER_PREFIX` (default `OPENCLAW_RUN:`). Slack event routing and mention-only behavior belongs in the Slack App/OpenClaw listener configuration, not in the music release skill docs.
- OpenClaw's continuous automation entry point is `docs/openclaw-next-release-planner.md`. After an upload/scheduled publish completes, the app asks OpenClaw to run that planner first. The planner reads `/youtube/status`, rotates connected non-excluded channels, reads the selected channel's concept planner/profile, avoids recent concept repetition, chooses the next channel/concept, then runs the automatic playlist publisher.
