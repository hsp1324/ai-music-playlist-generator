...
- Liked tracks are now standalone single candidates across all channels, not only playlist backfill priority. OpenClaw should search with `scripts/openclaw-release search-tracks --user-rating like --exclude-single-uploaded --full`, create a fresh Single Release, attach the existing track with `reuse-track`, and build special song-specific cover/thumbnail assets from lyrics/title/style. Successful single uploads are marked on the source track with `single_youtube_uploaded`, `single_youtube_video_id`, and `single_youtube_uploads` to prevent duplicate standalone uploads.
- Storylight OST is no longer an automated upload target. The human will manage that channel personally. OpenClaw should route any former Storylight-style no-vocal playful Japanese game/anime OST, cute arcade/fantasy-game BGM, and happy amusement/theme-park BGM releases to `Cinematic Pulse` instead, using the Storylight-style concept/visual lane when the music is playful rather than cinematic. Existing videos already published on Storylight OST must not be deleted or made private by automation. When migrating old Storylight uploads to Cinematic Pulse, only reuse an existing rendered MP4 that is still present locally and queue a YouTube upload with the corrected Cinematic schedule; do not create new audio-render or video-render placeholder workspaces just to represent old Storylight items. If the local MP4 is gone, archive/report that item for human decision instead of scheduling it.
...
- OpenClaw should not bake spectrum bars, waveform graphics, equalizers, or audio meters into the static cover or Gemini loop video; the app adds those during final render
...
3. generate cover
4. optionally generate a short Gemini clip
...
- OpenClaw playlist automation now uses step commands for continuous lookahead: upload/approve tracks, upload cover/thumbnail/visual source, render audio, queue external video render, then stop. Moving-video channels upload a Gemini provider loop video only when Gemini is available. HaruHaru, sundaze, Solwave Radio, and Tokyo Daydream Radio photorealistic Japanese hip-hop/R&B/rap releases skip loop-video upload and queue a 1080p `still_image` render with lower-left `editorial-lower-left` lyrics and lower-right `bars` spectrum. Tokyo Daydream Radio animated J-pop/city-pop/anime-pop releases use Gemini loop-video flow when available, otherwise the approved still-image fallback. Club Bloom skips loop-video upload too and queues a 1080p `still_image` render with lower-right `bars` spectrum only. Cinematic Pulse should try a Gemini loop video from its photorealistic cinematic or premium game-animation first frame; if Gemini is unavailable, on cooldown, blocked after safe retries, or cannot create a usable MP4 within the wait window, use the approved still-image fallback instead of Dreamina/Seedance/CapCut. The app asks OpenClaw again after the external worker completes render/upload so metadata/publish can finish while render workers process the next queued release. After a successful publish, the app also waits for the OpenClaw lock to clear and sends one compact backlog request as a fallback, so a partially prepared release stuck before video render, such as `audio_ready`, can continue instead of stalling the loop. OpenClaw can still run `scripts/openclaw-release auto-publish-single` for a human-approved end-to-end single upload. When `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads each video as a scheduled public release in `AIMP_YOUTUBE_SCHEDULE_TIMEZONE`; scripture releases on `BibliaCanto` have two app-managed daily slots, Old Testament at 07:00 and New Testament at 16:00; `불송` releases are scheduled daily at 07:00. Solo-piano BGM releases route to `Soft Hour Radio`; mainstream J-pop/Japanese pop/Tokyo pop and Japanese rap/hip-hop/R&B releases route to `Tokyo Daydream Radio`; Korean/K-pop vocal releases route to `HaruHaru`; playful no-vocal Japanese-style game/anime OST, arcade/fantasy-game BGM, and happy amusement park/theme-park BGM routes to `Cinematic Pulse` under the former Storylight-style lane; no-vocal cinematic orchestra, movie OST, film score, trailer, battle, emotional, mystery-tension, premium game orchestra, anime action-RPG OST, cinematic game score, gentle game orchestra, sweet fantasy game score, and lyrical game-focus music also routes to `Cinematic Pulse`; no-vocal EDM/house/techno/trance/workout/night-drive music routes to `Club Bloom`; Old Testament and New Testament Bible scripture-inspired music routes to `BibliaCanto`; Korean vocal music with an internal Buddhist-source lyric plan routes to `불송`; English/American pop playlist lanes route to `sundaze`, including pop-rock, country pop, Americana pop, indie/bedroom/alt-pop, singer-songwriter/folk-pop, soft rock, pop-punk, Y2K/recession pop, disco/funk pop, Afrobeats, Afropop, and Amapiano-pop; Latin/Spanish pop routes to `Solwave Radio`. `Storylight OST` and MusicSun are manual-only and excluded from continuous automation. Signal Room/Signal Desk/Midnight Cue names are retired unless explicitly revived.
- Cinematic Pulse lane consistency is required before metadata approval and publish. Former Storylight-style cute/playful anime-game/arcade/item-shop/theme-park BGM may route to Cinematic Pulse, but it must keep truthful former-Storylight game/anime/BGM titles, thumbnail phrases, descriptions, and visuals. Dark fantasy / epic battle / final battle / trailer / cinematic orchestra packaging is only allowed when the actual track list, especially the first 10 minutes, is cinematic/orchestral/film-score/game-orchestra enough to support that promise. If a mismatch is found after upload, make the video private or delete it as directed and mark the release as needing correction instead of treating it as successful.
...
- For 불송 and other vocal Suno work, voice/timbre cues are required, but OpenClaw should not add a mandatory full-verse or fixed-length listening gate before upload. Use lightweight vocal sanity checks: keep Suno setup clearly vocal/lyrics-based, avoid instrumental/no-vocal settings, and reject/regenerate only obvious no-vocal or non-usable vocal outputs caught from preview, metadata, or a very quick spot check. Failed no-vocal outputs from vocal releases should be marked disliked/reuse-disabled rather than reused as filler.
...
- BibliaCanto should rotate Bible release-level music lanes across uploads, but the lane must be trendy and secular-pop-adjacent, such as scripture hip-hop, Bible R&B, K-pop-inspired scripture pop, scripture rap-pop, trap-soul scripture songs, boom-bap Bible rap, alt-R&B scripture songs, neo-soul scripture songs, Afropop/Amapiano-pop scripture songs, dark street-pop scripture, or synth-pop scripture songs. New BibliaCanto music must not sound like Gospel music, worship, holy worship, church choir, hymns, praise band, CCM, congregational singing, piano worship ballads, or church-service music; OpenClaw should add those terms to Suno excluded styles. 불송 is diversity-first: the Buddhist source/theme stays internal as lyric inspiration only, but new releases should present as accessible mainstream Korean songs and rotate across coherent lanes such as hip-hop, boom bap, trap-soul, dark R&B, trip-hop, glitch-hop, drum and bass, UK garage, phonk, hyperpop, psychedelic soul, avant-jazz rap, spoken-word rap, new-age electronica, cinematic trap, and art-pop. One release must stay in one coherent lane so the YouTube title and description can truthfully name that genre. New default 불송 lyrics and public metadata should avoid overt Buddhist vocabulary/source names such as `불교`, `불경`, `법구경`, `금강경`, `반야심경`, `법화경`, `부처`, `자비`, `가르침`, `무상`, `무아`, `팔정도`, `정어`, and other 팔정도 세부 항목 unless the human explicitly asks for source-explicit packaging. Use mainstream song-first title shapes and store the verified source/theme in planning/app metadata. Already-uploaded/source-explicit 불송 videos belong in the YouTube playlist `불송`; new mainstream 불송 uploads belong in `노래`. New 불송 covers, thumbnails, and first-frame images should also avoid Buddhist-looking cues such as Buddha statues/faces, temples, monks, prayer poses, lotus, mandala, sutra pages, prayer beads, Dharma wheels, incense, shrines, and scripture text unless the human explicitly requests source-explicit visual packaging. 불송 Suno prompts/excluded styles should explicitly avoid trot, ppongjjak, old Korean cabaret-pop, trot vocal ornaments, and accordion/brass trot clichés.
...
- OpenClaw should generate static cover/thumbnail images with OpenAI GPT Image models, preferably `gpt-image-2` when available, not Dreamina or Gemini. Gemini is reserved for the moving visual clip; Dreamina, Seedance, and CapCut are disabled for new OpenClaw automation.
...
- OpenClaw should pass a short Gemini-generated MP4 via `--loop-video --loop-video-provider gemini` for normal moving-video publish automation while Gemini is available; the app stores this in `loop_video_provider` and `loop_video_history[].provider`. Dreamina, Seedance, and CapCut are disabled for new automation and must not be used as fallbacks. HaruHaru, sundaze, Solwave Radio, Tokyo Daydream Radio photorealistic Japanese hip-hop/R&B/rap releases, and Club Bloom are explicit still-image exceptions: do not upload `--loop-video`; pass `--allow-still-image-video --video-render-source-mode still_image`. Tokyo Daydream Radio animated J-pop/city-pop/anime-pop releases use Gemini loop video when available, but may use the human-approved still-image fallback when Gemini is unavailable, on cooldown, blocked after safe retries, or cannot create a usable MP4 within the wait window. Storylight-style and Cinematic Pulse moving-video releases should also try Gemini first; if Gemini cannot provide a usable clip, use the approved still-image fallback rather than waiting or switching providers. Browser automation should use Gemini by clicking `Create image` / the creation entry that accepts image+prompt, attaching the cover/first-frame image, generating video, downloading the MP4, and inspecting it. Gemini prompts should not mention duration; upload the generated Gemini MP4 as-is after inspection. Before clicking Generate in Gemini, OpenClaw must run `scripts/openclaw-release provider-video-start --release-id RELEASE_ID --first-frame FIRST_FRAME --provider gemini`. If Gemini is already generating for the same release and first-frame image, OpenClaw must wait and keep the OpenClaw lock alive. If Gemini has no usable MP4 after 20 minutes, mark it `timed_out` with `provider-video-finish`, then use the approved still-image fallback. Copyright/policy blocks before Gemini creates a video do not count and should be retried with safer prompts up to 10 times before falling back to still-image render. The app smooth loop crossfade defaults to 1.5 seconds, but Gemini-tagged loop videos use 2.0 seconds because Gemini clips are usually around 10 seconds. The first-frame image should be the cover or a dedicated first-frame image, not a text-heavy YouTube thumbnail, because large generated title text can flicker or disappear. The generated clip should end close to its opening composition so it can be reused across the full release.
- Existing releases that already have loop videos remain valid. New OpenClaw-created moving clips should be Gemini-only. Gemini clips are uploaded as generated.
- Loop-video upload validates only that the file is readable video. The app does not reject low-motion clips or alternate clip lengths; visual quality is handled by human/OpenClaw review before render/publish.
- Gemini prompts must preserve composition and any short style/passage phrase already present, but must not ask the provider to add or preserve channel names. Prompt visible state instead of conceptual labels: avoid words like `playlist`, `music visual`, `seamless loop`, `repeat`, `cyclic`, and scripture-framework terms that can imply multiple narrative stages. Prefer positive fixed-shot wording such as `single fixed camera shot`, `locked-off camera`, `one uninterrupted calm environmental take`, and `same composition from first to last frame`.
...
- Gemini content-safety or copyright rejections are retried with sanitized prompts. Gemini copyright/policy blocks do not count against successful-video quota unless Gemini actually creates a video. If Gemini is unavailable, on cooldown, blocked after safe retries, or cannot create a usable MP4 within the wait window, use the approved still-image fallback with `--allow-still-image-video --video-render-source-mode still_image` instead of deferring the release.
...
- HaruHaru, Tokyo Daydream Radio, sundaze, Solwave Radio, and 불송 now require one explicit genre lane per release, such as K-pop hip-hop, Korean R&B, explicit HaruHaru city-pop, Tokyo animated J-pop, Tokyo city-pop, anime-pop, Japanese rap, Japanese hip-hop, Tokyo R&B, Japanese neo-soul, sundaze Pop R&B, dance-pop, country pop, Americana pop, indie pop, Afropop, Amapiano-pop, Pop Latino, reggaeton pop, bachata pop, Latin R&B, 불송 hip-hop, 불송 boom-bap, 불송 trap-soul, 불송 R&B/soul, 불송 trip-hop, 불송 glitch-hop, 불송 DnB, 불송 hyperpop, 불송 art-pop, 불송 new-age electronica, or 불송 avant-jazz rap, with that lane named naturally in title/metadata and reflected in Suno style prompts. HaruHaru Korean/default titles should be click-led but not formula-led: compare at least the latest 15 HaruHaru titles and rotate between dialogue-hook-first, genre/use-case-first, immediate-payoff-first, mini-story-first, mood-identity-first, and searchable-lane-first shapes instead of repeatedly using `[hook] | [situation] 듣기 좋은 [genre] 노래모음`. Tokyo Daydream Radio alternates visual systems when practical: animated/anime moving-video for J-pop/city-pop/anime-pop lanes and photorealistic friend-taken Japanese street/lifestyle still-image renders for Japanese rap/hip-hop/R&B/neo-soul lanes. The cover/thumbnail mood must match the hook while remaining text-free by default where the channel profile says so. New default HaruHaru planning should favor hip-hop/R&B/trap/boom bap/neo-soul/dark street-pop, but if a release is explicitly or already city-pop then reuse/backfill should stay city-pop-related; non-city-pop HaruHaru releases must not be backfilled with city-pop tracks. HaruHaru, Tokyo Daydream Radio, sundaze, Solwave Radio, and 불송 reuse/backfill now require detailed-lane token matching, such as boom-bap to boom-bap, Tokyo R&B to R&B/neo-soul, country-pop to country-pop, reggaeton to reggaeton/urbano, and 불송 selected lane to same-lane material that truthfully fits, instead of broad `K-pop` / `J-pop` / `English pop` / `Latin pop` / channel-only overlap; if matching material is exhausted, do not force unrelated same-channel songs to reach one hour, and preserve the fresh/same-lane lead block before reused back-half tracks even if randomization is requested.
...
- Legacy integration exists, but OpenClaw automation should not use Dreamina, Seedance, or CapCut for new video generation.
- Moving-video automation uses Gemini only; if Gemini cannot provide a usable MP4, use the approved still-image fallback.
...
- legacy Dreamina loop-video integration, disabled for new OpenClaw automation
...
3. Improve failure recovery
   - retry publish jobs
   - recover Gemini loop-video jobs with still-image fallback when needed
