...
- Playlist automation should ask Suno for individual songs around 3:00-3:30, with 3:45 still in the preferred range. Tracks up to 4:20 are acceptable as returned output, but should not be the generation target. `scripts/openclaw-release auto-publish-playlist` rejects tracks over 260 seconds unless OpenClaw passes `--allow-long-track` after explicit human approval.
...
- Loop-video upload validates that the file is readable video, close to 10 seconds long, and has enough visible frame-to-frame motion. A technically valid but nearly static MP4 is rejected before it can be used for a long render.
