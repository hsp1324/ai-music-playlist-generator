...
- OpenClaw playlist automation can now run `scripts/openclaw-release auto-publish-playlist` to upload generated playlist tracks as approved, skip Slack per-track review spam, render audio/video, approve generated metadata, and upload privately to the selected connected YouTube channel. OpenClaw can also run `scripts/openclaw-release auto-publish-single` for a human-approved end-to-end private single upload. General BGM releases default to `Soft Hour Radio`; mainstream J-pop/Japanese pop/Tokyo pop releases route to `Tokyo Daydream Radio`; Korean/K-pop vocal releases route to `HaruHaru`; analytical/story/research/mystery BGM releases route to `Signal Room Radio` using legacy connected title `AI썰전` until the channel is manually renamed/reconnected; English/American pop routes to `sundaze`; Latin/Spanish pop routes to `Solwave Radio`.
...
- Playlist automation should ask Suno for complete tracks under 4 minutes, not exact target duration phrases in style fields. Pop-family vocal playlists should avoid 2:20-2:45 radio-edit outputs; OpenClaw should extend/regenerate those into complete songs unless the human explicitly accepts a short track. Fade-out endings are not desired; OpenClaw should ask for complete/resolved endings and regenerate tracks that fade out or sound cut off. Tracks up to 4:45 are acceptable as returned output when they end cleanly, but should not be the generation target. `scripts/openclaw-release auto-publish-playlist` rejects tracks under 180 seconds unless OpenClaw passes `--allow-short-track` and rejects tracks over 285 seconds unless OpenClaw passes `--allow-long-track`; both overrides require explicit human approval. Fade-out detection is intentionally not automated because false positives such as crowd, applause, or ambience endings are possible.
...
- OpenClaw's continuous automation entry point is `docs/openclaw-next-release-planner.md`. After a publish, the app asks OpenClaw to run that planner first. The planner reads `/youtube/status`, rotates connected non-excluded channels, reads the selected channel's concept planner/profile, avoids recent concept repetition, chooses the next channel/concept, then runs the automatic private playlist publisher.
