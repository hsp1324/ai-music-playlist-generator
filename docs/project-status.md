...
- The background worker also keeps disk usage under control: when `AIMP_STORAGE_ROOT` disk usage rises above `AIMP_LOCAL_VIDEO_CLEANUP_DISK_THRESHOLD_PERCENT` (default `50`), it deletes local rendered MP4 files for releases already uploaded to YouTube, oldest first, and records the deletion in release metadata.
