...
- Every channel now requires a consistent lower-left channel-name watermark on the video cover/first-frame and inside the Dreamina/Seedance loop video. The cover/first-frame should contain only that channel name; thumbnails should use large click text plus a smaller channel-name brand line whose size/style stays consistent with the cover watermark.
...
- The app can send a "make the next playlist" Slack request to the OpenClaw channel after a release is published. Configure `AIMP_OPENCLAW_SLACK_CHANNEL_ID`; the web UI shows `Request Next Playlist` on published releases, and `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_ON_PUBLISH=true` sends the request automatically after successful YouTube upload with per-video dedupe. Real OpenClaw task messages are prefixed with `AIMP_OPENCLAW_SLACK_TRIGGER_PREFIX` (default `OPENCLAW_RUN:`). OpenClaw's Slack listener must ignore ordinary channel messages that do not start with that prefix and do not explicitly mention the OpenClaw bot.
