...
- Playlist automation now avoids putting duration caps such as `less than 4 minutes` or `under 4 minutes` into Suno prompts, lyrics, style strings, or bracketed metatags unless a human explicitly asks for that cap. OpenClaw should instead prompt for a full-length complete song/cue with at least 2 minutes of developed structure: natural intro, verse/section flow, chorus or central motif where appropriate, bridge/breakdown when useful, and a resolved ending. Suno has no guaranteed minimum-duration field, so "2 minutes or longer" in style can help but is not enough by itself; validate the generated file after download. The helper rejects playlist tracks shorter than 2:00 by default and has no default maximum duration. Complete 5+ minute tracks are allowed on every channel.
...
- HaruHaru, sundaze, and Solwave Radio now require one explicit genre lane per release, such as K-pop hip-hop, Korean R&B, sundaze Pop R&B, dance-pop, country pop, Americana pop, indie pop, Afropop, Amapiano-pop, Pop Latino, reggaeton pop, bachata pop, or Latin R&B, with that lane named naturally in title/metadata and reflected in Suno style prompts.
