...
- Playlist automation should ask Suno for individual songs around 3:00-3:30, with 3:45 still in the preferred range. Pop-family vocal playlists should avoid 2:20-2:45 radio-edit outputs; OpenClaw should extend/regenerate those into complete songs unless the human explicitly accepts a short track. Fade-out endings are not desired; OpenClaw should ask for complete/resolved endings and regenerate tracks that fade out or sound cut off. Tracks up to 4:45 are acceptable as returned output when they end cleanly, but should not be the generation target. `scripts/openclaw-release auto-publish-playlist` rejects tracks under 180 seconds unless OpenClaw passes `--allow-short-track`, rejects tracks over 285 seconds unless OpenClaw passes `--allow-long-track`, and now rejects likely long fade-out endings unless OpenClaw passes `--allow-fade-out-track`; all overrides require explicit human approval.
