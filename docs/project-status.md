...
- Playlist automation now keeps Suno duration wording minimal: use only `less than 4 minutes` or `under 4 minutes` when a duration hint is needed. The helper allows playlist tracks up to 4:20 by default so complete songs are not trimmed or thrown away just because they run slightly long, and it no longer has a default lower-bound track duration.
