...
- Gemini/Dreamina/Seedance content-safety or copyright rejections are retried with sanitized prompts. Gemini copyright/policy blocks do not count against the 3 successful-video quota unless Gemini actually creates a video. Repeated failures stop the automation before render/publish unless the human explicitly accepts a still-image fallback and `--allow-still-image-video` is passed.
