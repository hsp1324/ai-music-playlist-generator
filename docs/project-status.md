...
- YouTube metadata can now store `ko`, `ja`, `en`, `es`, `vi`, `th`, `hi`, `fil`, `zh-CN`, and `zh-TW` localized title/description drafts. Releases should provide all ten where possible; the publish flow sends the selected default metadata language and sends the other localized title/description drafts as YouTube localizations.
...
- Playlist automation should ask Suno for individual songs around 3:00-3:30, with 3:45 still in the preferred range. Pop-family vocal playlists should avoid 2:20-2:45 radio-edit outputs; OpenClaw should extend/regenerate those into complete songs unless the human explicitly accepts a short track. Tracks up to 4:20 are acceptable as returned output, but should not be the generation target. `scripts/openclaw-release auto-publish-playlist` rejects tracks under 180 seconds unless OpenClaw passes `--allow-short-track`, and rejects tracks over 260 seconds unless OpenClaw passes `--allow-long-track`, both only after explicit human approval.
...
- The app can send a "make the next playlist" Slack request to the OpenClaw channel after a release is published. Configure `AIMP_OPENCLAW_SLACK_CHANNEL_ID`; the web UI shows `Request Next Playlist`, and `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_ON_PUBLISH=true` sends the request automatically after successful YouTube upload with per-video dedupe. `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_MAX_UPLOADS=N` caps the loop after N successful YouTube uploads; `0` means unlimited. Real app-originated OpenClaw task messages are prefixed with `AIMP_OPENCLAW_SLACK_TRIGGER_PREFIX` (default `OPENCLAW_RUN:`). Slack event routing and mention-only behavior belongs in the Slack App/OpenClaw listener configuration, not in the music release skill docs.
- OpenClaw's continuous automation entry point is `docs/openclaw-next-release-planner.md`. After a publish, the app asks OpenClaw to run that planner first. The planner rotates active channels, reads the selected channel's concept planner, avoids recent concept repetition, chooses the next channel/concept, then runs the automatic private playlist publisher.
