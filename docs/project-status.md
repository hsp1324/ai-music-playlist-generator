...
- Video render can also burn line-level lyric subtitles into the final MP4 when `video_lyrics_overlay_enabled=true` or OpenClaw passes `--lyrics-overlay`. The first implementation uses stored track lyrics plus the rendered track timeline to create approximate line cues, then applies ASS subtitles as the final ffmpeg pass. It is not word-level forced alignment yet, but the cue generator is isolated so a later ASR/forced-alignment backend can replace only the timing step. `AIMP_VIDEO_LYRICS_OVERLAY_FONT` controls the ASS font; render workers should have a CJK-capable font such as `fonts-noto-cjk` installed for Korean/Japanese lyric overlays.
