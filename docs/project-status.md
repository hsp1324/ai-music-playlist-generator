...
- The app keeps disk usage under control: when `AIMP_STORAGE_ROOT` disk usage rises above `AIMP_LOCAL_VIDEO_CLEANUP_DISK_THRESHOLD_PERCENT` (default `80`), it deletes local rendered MP4 files for releases already uploaded to YouTube and public, oldest first, and records the deletion in release metadata. The check runs periodically and immediately after OpenClaw loop-video uploads, external render-worker chunk uploads, and external render completion.
