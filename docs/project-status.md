...
- Visual asset rules are channel-specific and documented in `docs/openclaw-visual-assets.md`. `Tokyo Daydream Radio` / Japan / J-pop releases use the three-people back-view walking signature by default. `Soft Hour Radio` / default BGM / cafe / sleep / study / chill releases use calm atmospheric visuals by default and should not use the Tokyo three-person signature unless explicitly requested. Human requests for a different scene, subject, action, camera angle, object, animal, or character type override the selected channel default and should be applied consistently to cover, thumbnail, and loop video. The YouTube thumbnail should be generated from the final clean cover as a reference/edit derivative so characters, positions, outfit colors, lighting, palette, and background stay continuous. All generated visuals should stay animated/anime/illustrated/stylized, not photorealistic or live-action.
...
- OpenClaw can also pass an 8 second Dreamina/Seedance MP4 via `--loop-video`. The app stores it as `loop_video_path` and uses smooth 2 second forward crossfade looping during final video render instead of making OpenClaw export a one-hour video. Browser automation should use Dreamina/Seedance `2.0 Fast`, first-frame only, no Omni Reference, no last-frame reference, `16:9`, `720p`, and exactly `8 seconds` through UI controls. The first-frame image should be the clean text-free cover or a separate clean text-free image by default, not the YouTube text thumbnail, because generated text can flicker or disappear during looping. If the human explicitly requests video channel text, the first-frame image may include only the lower-left channel label.
- If the human explicitly requests channel text inside the moving video, OpenClaw may create the cover/first-frame with only a lower-left channel label such as `Tokyo Daydream Radio`, using GPT Image typography that matches the scene. Dreamina/Seedance prompts must preserve the exact text for the full clip, and OpenClaw should reject/regenerate clips where the label flickers, morphs, disappears, moves drastically, or becomes unreadable.
