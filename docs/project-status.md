...
- The default channel visual signature is three people seen from behind, walking forward away from the viewer into the scene. OpenClaw should apply this to every clean cover, text thumbnail, and Dreamina/Seedance first frame unless the human provides a specific visual/video request. Human requests for a different scene, subject, action, camera angle, object, animal, or character type override the default and should be applied consistently to cover, thumbnail, and loop video. The YouTube thumbnail should be generated from the final clean cover as a reference/edit derivative so characters, positions, outfit colors, lighting, palette, and background stay continuous. All generated visuals should stay animated/anime/illustrated/stylized, not photorealistic or live-action.
