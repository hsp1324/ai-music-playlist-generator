...
- Playlist automation aims for individual songs around 2:30-3:45, but tracks up to 4:20 are acceptable. `scripts/openclaw-release auto-publish-playlist` rejects tracks over 260 seconds unless OpenClaw passes `--allow-long-track` after explicit human approval.
