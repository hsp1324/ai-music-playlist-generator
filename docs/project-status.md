...
- Playlist automation now keeps Suno duration wording minimal: use only `less than 4 minutes` or `under 4 minutes` when a duration hint is needed. The helper allows playlist tracks up to 4:20 by default and no longer has a default lower-bound track duration.
