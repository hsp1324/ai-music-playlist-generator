...
- For `Storylight OST`, `Cinematic Pulse`, `Club Bloom`, and `The Old Verse`, use English as the main/default metadata language with `--default-language en`. For Buddhist `The New Verse`, use Korean as the main/default metadata language with `--default-language ko`.
...
Good `The Old Verse` examples: `[playlist] Genesis 1:1-5 Creation Songs | Old Testament Music for Worship and Reflection`; `[playlist] Matthew 1:18-25 Emmanuel Worship | New Testament Music for Prayer`; `[playlist] Genesis Scripture Jazz | Old Testament Music for Worship`.
Good `The New Verse` examples: `[playlist] 마음을 다스리는 불경 힙합 | 법구경에서 영감을 받은 한국어 랩`; `[playlist] 반야심경 R&B | 집착을 내려놓는 불교 노래`; `[playlist] Buddhist Jazz for Letting Go | Dhammapada-Inspired Songs`.
...
- If the selected channel is Storylight OST, Cinematic Pulse, Club Bloom, or The Old Verse, make the English file the default top-level title/description and approve with `--default-language en`. If the selected channel is Buddhist The New Verse, make the Korean file the default top-level title/description and approve with `--default-language ko`.
