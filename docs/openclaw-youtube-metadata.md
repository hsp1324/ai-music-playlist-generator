...
- For `Midnight Cue Radio` cinematic mystery/storytelling BGM releases, use English as the main/default metadata language with `--default-language en`. If the connected channel still appears as `AI썰전`, treat it as legacy Midnight Cue Radio and use the intended `Midnight Cue Radio` brand in metadata/visuals.
...
For Midnight Cue Radio mystery/storytelling BGM releases, write the main title/description in English, use `--default-language en`, and provide Korean, Japanese, English, Spanish, Vietnamese, Thai, Hindi, Filipino, Indonesian, Brazilian Portuguese, French, German, Arabic, Simplified Chinese, and Traditional Chinese localized versions.
...
Good `Midnight Cue Radio` example: `[playlist] Midnight Mystery Documentary BGM | Dark Cases, Archives and Investigation Music`.
...
- If the selected channel is Midnight Cue Radio or legacy AI썰전, make the English file the default top-level title/description and approve with `--default-language en`.
