...
- For playlist/BGM titles, include a clear listening situation in the title itself, not only in the description. Choose the situation from the actual concept and sound; do not repeat `study`, `work`, `walk`, or `rest` as defaults. Normal automated playlists now target 60+ minutes, so one-hour wording is acceptable only when the rendered duration is actually close to or above one hour. Good Korean BGM patterns when they genuinely fit: `1시간 공부할 때 듣기 좋은 숲길 BGM`, `1시간 산책·드라이브할 때 듣기 좋은 해변 BGM`, `1시간 작업할 때 틀어놓기 좋은 카페 피아노`.
...
scripts/openclaw-release approve-metadata \
  ...
  --title "[playlist] 1시간 조용한 카페 피아노 솔로 | 공부, 작업, 휴식할 때 듣기 좋은 잔잔한 음악" \
