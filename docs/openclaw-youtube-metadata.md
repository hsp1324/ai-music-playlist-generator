...
- For `Signal Room Radio` analytical/story/research/mystery BGM releases, use English as the main/default metadata language with `--default-language en`. If the connected channel still appears as `AI썰전`, treat it as legacy Signal Room Radio and use the intended `Signal Room Radio` brand in metadata/visuals.
...
For Signal Room Radio analytical/story/research/mystery BGM releases, write the main title/description in English, use `--default-language en`, and provide Korean, Japanese, English, Spanish, Vietnamese, Thai, Hindi, Filipino, Indonesian, Brazilian Portuguese, French, German, Arabic, Simplified Chinese, and Traditional Chinese localized versions.
...
For `sundaze`, `Solwave Radio`, `HaruHaru`, and `Signal Room Radio`, titles should read like curated editorial or `Essential` playlists: vivid situation/emotion + genre identity + listening use case. Do not use raw workspace names or short generic labels such as `Golden Hour Drive Pop`, `Ruta Dorada Pop`, `English Pop`, `Spanish Pop`, `Latin Pop`, `KPOP`, `Korean Pop`, `Mystery BGM`, `Dark Music`, `AI Debate`, or `Research BGM` by themselves.
...
Good `HaruHaru` example: `[playlist] 비 오는 서울 K-POP 믹스 | 늦은 밤 버스, 이별, 감성 보컬`.
Good `Signal Room Radio` examples: `[playlist] AI Debate Prep BGM | Research, Script Notes and Deep Focus Music`; `[playlist] Mystery Documentary BGM | Archive Room, Quiet Clues and Deep Focus`.
...
- If the selected channel is Signal Room Radio or legacy AI썰전, make the English file the default top-level title/description and approve with `--default-language en`.
