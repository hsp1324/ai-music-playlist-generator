...
- For playlist/BGM titles, include a clear listening situation in the title itself, not only in the description. Choose the situation from the actual concept and sound; do not repeat `study`, `work`, `walk`, or `rest` as defaults. Normal automated playlists ask OpenClaw for roughly 40 minutes of new audio and may render as 60+ minutes when same-channel, similar-genre reuse is available, so one-hour wording is acceptable only when the rendered duration is actually close to or above one hour. Good Korean BGM patterns when they genuinely fit: `공부할 때 듣기 좋은 숲길 BGM`, `산책·드라이브할 때 듣기 좋은 해변 BGM`, `작업할 때 틀어놓기 좋은 카페 피아노`.
