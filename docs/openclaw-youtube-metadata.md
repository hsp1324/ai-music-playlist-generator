...
- For `Storylight OST`, `Cinematic Pulse`, `Club Bloom`, `The Old Verse`, and `The New Verse`, use English as the main/default metadata language with `--default-language en`. If the connected channel still appears as legacy `AI썰전`, treat it as the old title for `Club Bloom` and use the intended `Club Bloom` brand in metadata/visuals.
...
Good `The Old Verse` examples: `[playlist] Genesis Creation Songs | Old Testament Music for Worship and Reflection`; `[playlist] Abraham Covenant Songs | Bible Music for Prayer, Hope and Waiting`.
Good `The New Verse` examples: `[playlist] Matthew Gospel Songs | New Testament Worship Music for Prayer`; `[playlist] Grace Worship Songs | New Testament Music for Hope and Reflection`.
...
- If the selected channel is Storylight OST, Cinematic Pulse, Club Bloom, The Old Verse, The New Verse, or legacy AI썰전-to-Club-Bloom work, make the English file the default top-level title/description and approve with `--default-language en`.
