...
- Do not put duration caps such as `less than 4 minutes` or `under 4 minutes` into Suno prompts, style fields, lyrics, or bracketed metatags unless the human explicitly asks for that cap. Those caps can make Suno end too early. Ask for a full-length complete song/cue of at least 2 minutes, with a natural intro, developed middle, and resolved ending instead. Still verify the downloaded duration because Suno has no guaranteed minimum-duration field.
...
- Do not put duration caps such as `less than 4 minutes` or `under 4 minutes` into Suno prompts, style strings, lyrics, or bracketed metatags unless the human explicitly asks for that cap. Ask for a full-length complete song/cue with at least 2 minutes of developed structure, but verify the downloaded duration because Suno has no guaranteed minimum-duration field. The helper rejects playlist tracks shorter than 2:00 by default and has no default maximum duration. Complete 5+ minute tracks are allowed on every channel.
...
- Create the final cover/first-frame first. Do not put channel names or logos on it. Then create the YouTube thumbnail from that exact final cover as an image-to-image edit/reference derivative. Do not make the thumbnail as a fresh unrelated generation. For HaruHaru, keep both images photorealistic, natural, and text-free by default, with casual friend-taken travel/cafe snapshot energy instead of studio, fashion campaign, or tight straight-on AI beauty portrait energy. For Solwave Radio, keep both images photorealistic and friend-taken smartphone/Instagram-like; avoid professional photographer shoots, glossy campaigns, studio portraits, and tight straight-on AI-beauty close-ups. For `불송`, create one Buddhist cover/first-frame image with a short Korean passage/theme + style phrase when useful and reuse that exact image as the YouTube thumbnail when appropriate.
...
- For `불송`, the cover, YouTube thumbnail, first-frame, and loop video first frame are the same visual package. It may contain one short Korean passage/theme + style phrase, but never `불송`, title sentences, sutra paragraphs, captions, UI, logos, or unrelated words.
...
- Keep every static visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Solwave Radio, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru and Solwave Radio are photorealistic lifestyle still-image by default, with Solwave using a casual friend-taken smartphone look.
- The cover should be the clean requested scene without channel names. The YouTube thumbnail should usually use the same composition plus large readable click text, except HaruHaru where it should usually stay text-free. Solwave Radio should keep a photorealistic friend-taken phone-photo lifestyle look and use at most one short integrated Latin/Spanish lane phrase.
...
- For `Soft Hour Radio`, use thumbnail wording such as `DEEP SLEEP`, `CAFE PIANO`, `FOCUS MUSIC`, `RAINY NIGHT`, `STUDY BGM`, `LOFI`, or `CALM READING`.
...
- For `BibliaCanto`, use passage-aware thumbnail wording such as `Genesis 1:1-5`, `Matthew 1:18-25`, `GENESIS SONGS`, `GOSPEL SONGS`, `BIBLE MUSIC`, `PSALMS MUSIC`, `SCRIPTURE SONGS`, or a clear worship lane. Do not add `BIBLIACANTO` or any channel name to the visual.
- For `불송`, use the same cover, YouTube thumbnail, first-frame, and loop-video first-frame package. It may contain one short upper-left Korean passage/theme + style phrase such as `팔정도 명상팝`, `자비 트립합`, or `무상 불교 재즈`, but never the `불송` channel name.
...
- `--loop-video /absolute/path/to/loop-video.mp4`: Gemini/Dreamina/Seedance visual clip for the rendered video on moving-video channels. Gemini clips are uploaded as generated after inspection; Dreamina/Seedance clips should be generated with the duration control set to exactly 7 seconds, except `불송` clips which use 6 seconds. HaruHaru and Solwave Radio are not moving-video channels by default; omit `--loop-video`.
...
- Use Gemini only for moving loop-video generation, not for static cover or YouTube thumbnail creation. HaruHaru and Solwave Radio skip this workflow in normal automation.
...
- For HaruHaru and Solwave Radio photorealistic still images, visible adult faces are allowed and preferred when natural. Do not hide faces by default. Reject/regenerate only for explicit, minor-coded, celebrity-like, policy-blocked, badly distorted, or visually unusable results.
...
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips are normally 7 seconds, `불송` clips are 6 seconds, and Gemini clips are uploaded as generated. For HaruHaru and Solwave Radio, skip loop video and use `still_image` render flags.
...
- For playlist work, confirm every uploaded `duration_seconds` is at least 120. There is no default maximum track length; complete 5+ minute tracks are acceptable.
...
- For HaruHaru, sundaze, and Solwave Radio playlist releases, choose one explicit genre lane before Suno generation and keep the whole playlist in that lane. sundaze lanes include Pop R&B, dance-pop, synth-pop, pop-rock, country pop, Americana pop, indie/bedroom/alt-pop, singer-songwriter/folk-pop, soft rock, pop-punk, Y2K/recession pop, disco/funk pop, Afrobeats, Afropop, and Amapiano-pop. Name that lane in the public title/metadata when accurate instead of writing a generic mixed-pop title.
