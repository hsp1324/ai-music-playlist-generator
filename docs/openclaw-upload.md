...
For higher-level OpenClaw skill instructions, including "make one single", "build a playlist with roughly 15 minutes of new audio on normal channels before app-side reuse and roughly 40 minutes on scripture/Buddhist channels", and "write YouTube metadata", see [openclaw-skills.md](openclaw-skills.md).
...
- `--loop-video /absolute/path/to/loop-video.mp4`: Gemini/Dreamina/Seedance visual clip for the rendered video on moving-video channels. Gemini clips are uploaded as generated after inspection; Dreamina/Seedance clips should be generated with the duration control set to exactly 7 seconds, except `불송` clips which use 6 seconds. HaruHaru, sundaze, and Solwave Radio are not moving-video channels by default; omit `--loop-video`.
...
- Use Dreamina/Seedance `2.0 Fast` for normal moving-video releases.
...
- Set duration to the channel's required duration and re-check this visible control immediately before clicking Generate. Normal releases use exactly `7 seconds`; `불송` uses exactly `6 seconds`. If the duration control is not visible when the Seedance/Dreamina screen first opens, gently drag/scroll the settings/control row to the right until the duration option is visible, then set the required duration.
- Do not click Generate while the duration control is hidden, while it shows the wrong duration, or while you are unsure. Do not create an initial 5 second clip, draft/test clip, or any other wrong-duration clip first.
- Generate exactly one short MP4 with the channel's required duration.
- Download the generated MP4 to the VM or OpenClaw workspace.
...
- Do not include duration values such as `7 seconds` or `6 seconds`, `16:9`, `720p`, `loop`, `seamless loop`, `repeat`, or `cyclic` in the prompt. Do not mention duration in Gemini prompts. These words can make Seedance/Dreamina/Gemini create a shorter repeated segment inside the clip.
...
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips are normally 7 seconds, `불송` clips are 6 seconds, and Gemini clips are uploaded as generated. For HaruHaru, sundaze, and Solwave Radio, skip loop video and use `still_image` render flags.
...
- For Gemini, use the Gemini-first `Create image` / image+prompt workflow above for moving-video channels, attach the cover/first-frame image, do not mention duration, count only successful video generations toward the 3 videos per 24 hour quota, and upload with `--loop-video-provider gemini`. For Dreamina/Seedance, use `2.0 Fast`, first-frame only, no Omni Reference, no last-frame reference, `16:9`, `720p`, and the channel's required duration through UI controls, then upload with `--loop-video-provider dreamina` or `--loop-video-provider seedance`. If the duration control is hidden on entry, drag/scroll the settings/control row to the right until the control is visible before Generate. Normal moving-video releases use exactly `7 seconds`; `불송` uses exactly `6 seconds`. HaruHaru, sundaze, and Solwave Radio do not use Gemini/Dreamina/Seedance in normal automation. Do not put those settings in the prompt.
- For normal OpenClaw auto-publish work, verify the MP4 after download. If Seedance/Dreamina did not produce the requested channel duration, discard/regenerate unless the human explicitly accepts it and OpenClaw passes `--allow-short-loop-video`. For Gemini, inspect the generated MP4 and upload it as-is when text, framing, and motion are acceptable.
