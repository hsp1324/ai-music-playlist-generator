...
- For `The Old Verse` and `The New Verse`, use `docs/openclaw-scripture-sequence.md` and the app-owned `scripts/openclaw-release openclaw-scripture-reserve` / `openclaw-scripture-complete` helpers. Do not use a local scripture ledger or local mismatch checks during normal automation.
...
- If `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads with YouTube `status.publishAt` for the next free daily 07:00 slot in `AIMP_YOUTUBE_SCHEDULE_TIMEZONE` for the selected channel. If that selected channel already has a scheduled app upload on that local date, the app uses the following day for that same channel. Other channels' schedules do not block this channel. OpenClaw should not manually change YouTube visibility in Studio.
