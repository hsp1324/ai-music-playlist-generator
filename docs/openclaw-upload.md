...
- Keep every static visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, sundaze, Solwave Radio, Club Bloom, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru, sundaze, Solwave Radio, and Club Bloom are photorealistic still-image by default, with sundaze/Solwave using a casual friend-taken smartphone look and Club Bloom using premium nightlife photography.
...
- For `불송`, use the same cover, YouTube thumbnail, first-frame, and loop-video first-frame package. It may contain one short upper-left Korean passage/theme + hip-hop style phrase such as `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩`, but never the `불송` channel name and never obscure coined genre labels.
