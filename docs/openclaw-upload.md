...
- In Suno style fields, do not write exact target duration phrases; use `less than 4 minutes` plus `resolved ending, no fade-out` instead.
...
- Playlist track generation should avoid short 2:20-2:45 outputs for one-hour playlists. Ask for a natural complete track under 4 minutes rather than forcing exact duration wording in the style field. Anything up to 4:45 is acceptable as returned output when it has a complete ending, but OpenClaw should not intentionally ask Suno for long tracks. `auto-publish-playlist` rejects tracks under `180` seconds unless `--allow-short-track` is passed and rejects tracks over `285` seconds unless `--allow-long-track` is passed. Use these override flags only when the human explicitly accepts the shorter/longer track.
- Do not request or accept fade-out endings for Suno playlist tracks. Prompts, style strings, vocal lyrics, and bracketed instrumental metatags should say `no fade-out`, `complete ending`, `final cadence`, `resolved ending`, `hard ending`, `clear final chord`, or `final hit` where appropriate. A slightly longer complete song is preferred over a track that fades out mid-idea.
- Before upload, check the final 20-30 seconds of every playlist track. Reject/regenerate tracks where the music keeps going but the volume gradually drops, where a chorus/loop fades out just to fit a duration, or where the ending sounds cut off. The helper does not automatically reject fade-out endings because ambience, crowd, applause, or cinematic tail endings can cause false positives; this check is a human/OpenClaw quality decision.
...
For every playlist, ask Suno for a complete track under 4 minutes. For J-pop/K-pop/English pop/Latin pop/Spanish pop, ask for a complete song structure, not a short radio edit: intro, verse, pre-chorus, chorus, second verse, bridge or instrumental break, final chorus, and a resolved outro with a clear final chord/hit. Do not request fade-outs, slow fades, trailing volume-lowering endings, or exact target-duration style wording. A returned track around 4:04 is fine if it ends cleanly; replace any track under roughly three minutes, any fade-out/cut-off track, or any track over 4:45 before final publish unless the human explicitly approves it.
...
- For `Signal Room Radio` or legacy `AI썰전`, use the Signal Room Radio profile. This is an instrumental analytical/story BGM channel for research, debate prep, documentary, mystery, and focused work. It is not a narrated talk/debate channel in the current music automation pipeline.
...
- For `Signal Room Radio`, use thumbnail wording such as `AI DEBATE`, `RESEARCH`, `SCRIPT NOTES`, `FACT CHECK`, `DEEP WORK`, `DATA REVIEW`, `MYSTERY`, `DOCUMENTARY`, or `SIGNAL ROOM`, with smaller `SIGNAL ROOM RADIO` branding.
...
- For `Tokyo Daydream Radio`, `HaruHaru`, `Signal Room Radio`, `sundaze`, `Solwave Radio`, mainstream J-pop/Japanese pop, K-pop/Korean pop, English pop, Latin/Spanish pop, analytical/story/research/mystery BGM, or similar pop-family/story-BGM releases, always write all fifteen language versions: Korean, Japanese, English, Spanish, Vietnamese, Thai, Hindi, Filipino, Indonesian, Brazilian Portuguese, French, German, Arabic, Simplified Chinese, and Traditional Chinese.
- Use Korean as the default upload metadata for Tokyo/Soft Hour/HaruHaru unless the channel profile says otherwise. Use `--default-language en` for `Signal Room Radio` and `sundaze`; use `--default-language es` for `Solwave Radio`.
...
- Use `Signal Room Radio` for analytical/story/research BGM, AI debate prep, documentary BGM, mystery BGM, data review, fact-checking, newsroom focus, or similar instrumental story-background concepts. If the connected channel still appears as `AI썰전`, treat it as legacy Signal Room Radio.
...
For non-Japan releases, use the selected channel profile. Korean/K-pop goes to "HaruHaru", analytical/story/research/mystery BGM goes to "Signal Room Radio" (legacy "AI썰전"), English/American pop goes to "sundaze", Latin/Spanish pop goes to "Solwave Radio", and default BGM/background goes to "Soft Hour Radio" unless the human says otherwise.
...
- For playlist work, confirm every uploaded `duration_seconds` is at or below 285 unless the human explicitly approved a longer track.
