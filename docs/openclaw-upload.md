...
- Use Dreamina/Seedance `1.0 Fast` for normal moving-video releases.
...
- Leave the last-frame input empty. Do not upload a last-frame reference; adding both first and last frames makes Dreamina switch back to `2.0 Fast` and wastes Seedance credits.
- Set ratio to `16:9` when selectable.
- Set quality to `720p` when selectable.
- Set duration to exactly `10 seconds` and re-check this visible control immediately before clicking Generate. Seedance/Dreamina `1.0 Fast` offers `5 seconds` or `10 seconds`; always choose `10 seconds`. If the duration control is not visible when the Seedance/Dreamina screen first opens, gently drag/scroll the settings/control row to the right until the duration option is visible, then set `10 seconds`.
- Do not click Generate while the duration control is hidden, while it shows 5 seconds, while the model has switched to `2.0 Fast`, or while you are unsure. Do not create an initial 5 second clip, draft/test clip, or any other wrong-duration clip first.
...
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips use `1.0 Fast`, first-frame only, `720p`, exactly 10 seconds, and Gemini clips are uploaded as generated. For HaruHaru, sundaze, Solwave Radio, Club Bloom, and 불송, skip loop video and use `still_image` render flags.
...
- For Gemini, use the Gemini-first `Create image` / image+prompt workflow above for moving-video channels, attach the cover/first-frame image, do not mention duration, count only successful video generations toward the 3 videos per 24 hour quota, and upload with `--loop-video-provider gemini`. For Dreamina/Seedance, use `1.0 Fast`, first-frame/start-frame only, no Omni Reference, no last-frame/end-frame reference, `16:9`, `720p`, and exactly `10 seconds` through UI controls, then upload with `--loop-video-provider dreamina` or `--loop-video-provider seedance`. Do not upload both first and last frames, because Dreamina switches that setup back to `2.0 Fast`. If the duration control is hidden on entry, drag/scroll the settings/control row to the right until the control is visible before Generate. HaruHaru, sundaze, Solwave Radio, and Club Bloom do not use Gemini/Dreamina/Seedance in normal automation. Do not put those settings in the prompt.
