...
- Use `Seedance 2.0 Mini` in Dreamina/Seedance for normal moving-video releases.
...
- Set duration to exactly `10 seconds` and re-check this visible control immediately before clicking Generate. `Seedance 2.0 Mini` should be set to `10 seconds`; if the UI offers `5 seconds` or `10 seconds`, always choose `10 seconds`. If the duration control is not visible when the Seedance/Dreamina screen first opens, gently drag/scroll the settings/control row to the right until the duration option is visible, then set `10 seconds`.
...
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips use `Seedance 2.0 Mini`, first-frame only, `720p`, exactly 10 seconds, and Gemini clips are uploaded as generated. For HaruHaru, sundaze, Solwave Radio, Club Bloom, and 불송, skip loop video and use `still_image` render flags.
