...
For higher-level OpenClaw skill instructions, including "make one single", "build a playlist with roughly 10 minutes of new audio on normal channels before app-side reuse and roughly 40 minutes on scripture/Buddhist channels", and "write YouTube metadata", see [openclaw-skills.md](openclaw-skills.md).
...
- For `Club Bloom`, use the Club Bloom profile for no-vocal EDM, house, techno, trance, festival, workout, night-drive, gaming, club, and party-energy releases. Each playlist must choose one club style lane and stay within it. Club Bloom now defaults to photorealistic club/bar/lounge/festival still-image cover/thumbnail visuals and app `still_image` video render with lower-right `bars` spectrum, not provider loop video. Unless the human asks otherwise, Club Bloom visuals should look like hot adult nightlife scenes in places where club music naturally plays, with attractive clearly adult women in revealing YouTube-safe club fashion, not abstract neon.
...
- For `불송`, the cover, YouTube thumbnail, first-frame, and loop video first frame are the same contemporary Buddhist music visual package. Rotate fresh subjects such as adult temple-stay listeners, Hongdae/Seoul streetwear practitioners, Buddha/statue plus modern listener scenes, duet pairs, or animated/stylized hip monk concepts. It may contain one short Korean passage/theme + hip-hop style phrase, such as `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩`, but never `불송`, title sentences, sutra paragraphs, captions, UI, logos, unrelated words, or obscure coined genre labels.
...
- Keep every static visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, sundaze, Solwave Radio, Club Bloom, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru, sundaze, Solwave Radio, and Club Bloom are photorealistic still-image by default, with sundaze/Solwave using a casual friend-taken smartphone look and Club Bloom using premium nightlife photography. 불송 uses contemporary Buddhist music visuals and should not repeat stale Buddha-only, old sutra-desk, or background-only temple images.
- The cover should be the clean requested scene without channel names. The YouTube thumbnail should usually use the same composition plus large readable click text, except HaruHaru where it should usually stay text-free. sundaze and Solwave Radio should keep a photorealistic friend-taken phone-photo lifestyle look and use at most one short integrated lane phrase. Club Bloom should keep photorealistic premium nightlife energy and use at most one short integrated club lane phrase.
...
- For `Club Bloom`, use style-specific thumbnail wording such as `DEEP HOUSE`, `TECH HOUSE`, `MELODIC TECHNO`, `TRANCE MIX`, `BASS HOUSE`, `FESTIVAL EDM`, `WORKOUT EDM`, `UK GARAGE`, `LIQUID DNB`, `TROPICAL HOUSE`, `AFRO HOUSE`, `SYNTHWAVE DRIVE`, or `CLUB MIX`.
- For `Club Bloom`, reject visual assets that are too mild for a club channel: calm abstract neon, empty low-energy backgrounds, polite lounge imagery, low-contrast thumbnails, random glowing objects, or any image that lacks real nightlife/party energy. The cover and thumbnail should usually show attractive clearly adult women in revealing YouTube-safe club outfits at a place where club music naturally plays: nightclub, premium bar, lounge, rooftop club, beach club, pool party, festival VIP area, DJ booth, dance floor, neon city terrace, or yacht/harbor party. Reject full nudity, exposed nipples/genitals, sexual acts, minors, teen-coded styling, school uniforms, fetish framing, celebrity likenesses, protected brands, or porn-style composition.
...
- For `불송`, use the same cover, YouTube thumbnail, first-frame, and loop-video first-frame package, but rotate the subject package across contemporary Buddhist hip-hop looks rather than repeating one old devotional scene. It may contain one short upper-left Korean passage/theme + hip-hop style phrase such as `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩`, but never the `불송` channel name and never obscure coined genre labels.
...
- Normal moving-video auto-publish must include a provider-generated `--loop-video` from Gemini, Dreamina, or Seedance. Do not use the thumbnail image, a text-heavy image, an app-rendered still image, or a locally synthesized motion loop as the moving video visual. A still-image fallback is allowed only when the channel profile requires it, as HaruHaru, sundaze, Solwave Radio, and Club Bloom now do, or when the human explicitly requests it, and then OpenClaw must pass `--allow-still-image-video`. For HaruHaru, sundaze, and Solwave Radio, queue `--video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`; the MP4 uses lower-left lyrics and lower-right spectrum. For Club Bloom, queue `--video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars` without lyric overlay; the MP4 uses the still image plus lower-right spectrum. `Cinematic Pulse` also uses this loop-video path by default; use photorealistic cinematic first-frame art, a subtle provider loop, `--video-render-source-mode loop_video`, `--video-render-resolution 720p`, and `--video-spectrum-overlay-style bars` unless a human asks otherwise.
...
- If Gemini says the daily video limit is reached or OpenClaw knows 3 successful Gemini videos were generated less than 24 hours ago, stop using Gemini for now and use Dreamina/Seedance for the loop video on moving-video channels. If Dreamina/Seedance also cannot create the video, defer this release until the Gemini cooldown clears instead of rendering with a missing loop video.
...
- If Gemini quota is already exhausted, do not use `--allow-still-image-video` unless the channel profile requires it, as HaruHaru, sundaze, Solwave Radio, and Club Bloom now do, or the human explicitly approves it. Leave moving-video releases unreleased/unrendered, post `scripts/openclaw-release slack-notify --text "Gemini 3개 영상 쿼터가 끝났고 Dreamina/Seedance도 실패해서 이 릴리즈의 loop video를 보류합니다. 24시간 쿨다운 후 Gemini로 먼저 다시 만들겠습니다. RELEASE_TITLE"`, and resume that release first when Gemini can make videos again.
...
- Use the selected channel profile for subject/action/motion. Prefer positive fixed-shot language such as `single fixed camera shot`, `locked-off camera`, and `same composition from first to last frame`. Always require no channel names, no extra text overlays, no subtitles, no logos, and no UI. Require no photorealism for the normal illustrated/anime/stylized channels; for `Cinematic Pulse`, use original photorealistic cinematic film-still / premium movie-poster realism instead. Solwave Radio normally skips motion generation, but if the human explicitly requests motion, preserve its photorealistic Latin/Spanish friend-taken phone-photo lifestyle style.
...
- a separate YouTube thumbnail image with readable text for normal moving-video channels; for HaruHaru, use a text-free photorealistic lifestyle thumbnail; for Solwave Radio, use a photorealistic friend-taken phone-photo lifestyle thumbnail with one short integrated Latin/Spanish lane phrase when useful; for Club Bloom, use a photorealistic nightlife thumbnail with a short integrated club lane phrase when useful; for `불송`, reuse the same cover/first-frame package as the thumbnail
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips use `1.0 Fast`, first-frame only, `720p`, exactly 10 seconds, and Gemini clips are uploaded as generated. For HaruHaru, sundaze, Solwave Radio, and Club Bloom, skip loop video and use `still_image` render flags.
