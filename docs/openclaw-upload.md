...
- For 불송 audio, male lead vocals, male duet, male/female duet, female lead vocals, and mixed vocals are all allowed. Do not infer vocal gender from the female-led visual rule. Preserve the actual intended vocal concept in `--style`, especially when two male voices, a male lead, or a duet is part of the song.
...
- For `불송`, use the Buddhist profile for Buddhist scripture-inspired modern vocal music. New 불송 releases are hip-hop-first by default; use Korean rap/hip-hop lanes unless the human explicitly asks for another lane or the release is already in progress. Korean lyrics are the default, and the app schedules uploads public daily at 07:00 Asia/Seoul.
- For `불송`, the cover, YouTube thumbnail, first-frame, and still-image render source are the same contemporary Buddhist music visual package. Every new 불송 visual should be female-led by default: use clearly adult temple-stay women, Hongdae/Seoul streetwear women practitioners/listeners, Buddha/statue plus a modern woman listener scene, two adult women for duet songs, or female-coded animated/stylized Buddhist practitioner concepts. Do not use men, male monks, male rappers, male listeners, or male-only visual pair/couple imagery unless the human explicitly asks for a male visual subject in the current request. This visual rule does not ban male vocals or male duet audio. It may contain one short Korean passage/theme + hip-hop style phrase, such as `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩`, but never `불송`, title sentences, sutra paragraphs, captions, UI, logos, unrelated words, or obscure coined genre labels.
...
- For `불송`, use the same cover, YouTube thumbnail, first-frame, and still-image render source package, but rotate the subject package across contemporary female-led Buddhist hip-hop looks rather than repeating one old devotional scene. The primary visible human subject should be a clearly adult woman; do not use male visual subjects unless the human explicitly requests one in the current request. This visual rule does not restrict male lead vocals or male duet audio. It may contain one short upper-left Korean passage/theme + hip-hop style phrase such as `법구경 힙합`, `불경 힙합`, `마음챙김 랩`, `자비 힙합`, or `반야심경 랩`, but never the `불송` channel name and never obscure coined genre labels.
...
- Use `불송` for Buddhist scripture-inspired Korean hip-hop/rap vocal music: Dhammapada/법구경-inspired songs, Heart Sutra/반야심경-inspired songs, 불교 힙합, 불경 힙합, mindful hip-hop, Korean Buddhist rap, mellow boom bap, Buddhist hip-hop soul, restrained Buddhist trap-soul, and modern sutra-inspired Korean rap. Use a non-hip-hop Buddhist lane only when the human explicitly asks or when finishing an already-started release in that lane. Avoid trot, ppongjjak, and old Korean cabaret-pop unless explicitly requested. The app schedules these uploads public daily at 07:00 Asia/Seoul.
...
- For `불송` Suno work, choose a hip-hop-first Buddhist lane such as `불교 힙합`, `불경 힙합`, mindful hip-hop, Korean Buddhist rap, mellow boom bap, Buddhist hip-hop soul, or restrained Buddhist trap-soul. Do not use obscure coined genre wording in titles, thumbnail text, descriptions, or tags; use plain public wording such as `불교 힙합`, `불경 힙합`, `법구경 힙합`, `반야심경 랩`, `Buddhist hip-hop`, or `Korean Buddhist rap`. Generate roughly 10 minutes of strong new Buddhist hip-hop/rap, put the strongest hook/rap/vocal entry first, then fill toward one hour only with previous same-channel, same-lane 불송 tracks, liked first. Do not add unrelated Buddhist jazz, trot, acoustic, cinematic, or meditation tracks just to reach one hour; publish shorter if the matching pool is exhausted. Exclude trot/ppongjjak explicitly with `--exclude-style` or Suno Advanced Options: `trot, Korean trot, ppongjjak, 뽕짝, 트로트, trot vocal, cabaret trot, old Korean trot, two-beat trot bounce, accordion trot, brass trot`. Reject/regenerate outputs that strongly sound like trot or old Korean cabaret-pop.
