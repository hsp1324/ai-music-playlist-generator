...
- `--cover /absolute/path/to/video-cover.png`: playback visual and, for moving-video channels, Gemini/Dreamina/Seedance first-frame reference. For HaruHaru, this is the `still_image` render source. It must not contain channel names or logos. If text is useful, use only a short style, genre, use-case, or passage phrase; HaruHaru should not add style text by default.
- `--thumbnail /absolute/path/to/youtube-thumbnail.png`: YouTube click thumbnail. Use short readable click text on channels that call for text and never add the selected channel name or logo. For HaruHaru, use a text-free photorealistic lifestyle thumbnail derived from the cover by default. For `불송`, pass the same cover/first-frame package as the thumbnail or use `--allow-cover-as-thumbnail`.

Do not rely on the app's generated draft cover for YouTube upload. Do not reuse the cover as the thumbnail unless the human explicitly approves one image for both roles or the selected profile uses one image package (`불송`). The thumbnail is for clicks and should have concise text except on HaruHaru, where it should stay like a natural text-free lifestyle photo; the cover is the video first-frame reference or still-image render source and should stay cleaner.
...
- Create the final cover/first-frame first. Do not put channel names or logos on it. Then create the YouTube thumbnail from that exact final cover as an image-to-image edit/reference derivative. Do not make the thumbnail as a fresh unrelated generation. For HaruHaru, keep both images photorealistic, natural, and text-free by default. For `불송`, create one Buddhist cover/first-frame image with a short Korean passage/theme + style phrase when useful and reuse that exact image as the YouTube thumbnail when appropriate.
...
- For `HaruHaru` or Korean/K-pop releases, use the HaruHaru profile. It now defaults to photorealistic Korean lifestyle still-image cover/thumbnail visuals and app `still_image` video render, not provider loop video. Lyrics are Korean by default.
...
- Keep every static visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru is photorealistic lifestyle still-image by default.
- The cover should be the clean requested scene without channel names. The YouTube thumbnail should usually use the same composition plus large readable click text, except HaruHaru where it should usually stay text-free.
...
- For `HaruHaru`, do not add thumbnail wording by default. The thumbnail should usually be a premium photorealistic Korean lifestyle photo derived from the cover. If the human explicitly requests text later, keep it small, integrated, and never use the channel name.
...
- `--loop-video /absolute/path/to/loop-video.mp4`: Gemini/Dreamina/Seedance visual clip for the rendered video on moving-video channels. Gemini clips are uploaded as generated after inspection; Dreamina/Seedance clips should be generated with the duration control set to exactly 7 seconds, except `불송` clips which use 6 seconds. HaruHaru is not a moving-video channel by default; omit `--loop-video`.
...
- Normal moving-video auto-publish must include a provider-generated `--loop-video` from Gemini, Dreamina, or Seedance. Do not use the thumbnail image, a text-heavy image, an app-rendered still image, or a locally synthesized motion loop as the moving video visual. A still-image fallback is allowed only when the channel profile requires it, as HaruHaru now does, or when the human explicitly requests it, and then OpenClaw must pass `--allow-still-image-video`. For HaruHaru, queue `--video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. `Cinematic Pulse` also uses this loop-video path by default; use photorealistic cinematic first-frame art, a subtle provider loop, `--video-render-source-mode loop_video`, `--video-render-resolution 720p`, and `--video-spectrum-overlay-style bars` unless a human asks otherwise.
...
- Use Gemini only for moving loop-video generation, not for static cover or YouTube thumbnail creation. HaruHaru skips this workflow in normal automation.
...
- For HaruHaru photorealistic still images, visible adult faces are allowed and preferred when natural. Do not hide faces by default. Reject/regenerate only for explicit, minor-coded, celebrity-like, policy-blocked, badly distorted, or visually unusable results.
...
- Use the selected channel profile for subject/action/motion. Prefer positive fixed-shot language such as `single fixed camera shot`, `locked-off camera`, and `same composition from first to last frame`. Always require no channel names, no extra text overlays, no subtitles, no logos, and no UI. Require no photorealism for the normal illustrated/anime/stylized channels; for `Cinematic Pulse`, use original photorealistic cinematic film-still / premium movie-poster realism instead.
...
- a separate YouTube thumbnail image with readable text for normal moving-video channels; for HaruHaru, use a text-free photorealistic lifestyle thumbnail; for `불송`, reuse the same cover/first-frame package as the thumbnail
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips are normally 7 seconds, `불송` clips are 6 seconds, and Gemini clips are uploaded as generated. For HaruHaru, skip loop video and use `still_image` render flags.
...
Pass exactly one --audio/--title/--lyrics-file/--style per auto-publish-single run, plus one --exclude-style if excluded styles/negative tags were used. If two Suno outputs are both good, create separate cover/thumbnail/loop-video assets and run auto-publish-single twice for moving-video channels. For HaruHaru, create separate photorealistic still cover/thumbnail assets and pass `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left` instead of `--loop-video`. For `불송`, pass the same cover/first-frame package as `--thumbnail` or use `--allow-cover-as-thumbnail`.
```
