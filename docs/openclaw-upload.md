...
- Set duration to exactly `7 seconds` and re-check this visible control immediately before clicking Generate. If the duration control is not visible when the Seedance/Dreamina screen first opens, gently drag/scroll the settings/control row to the right until the duration option is visible, then set it to `7 seconds`. For `불송`, set duration to `6 seconds`.
...
- For Gemini, use the Gemini-first `Create image` / image+prompt workflow above, attach the cover/first-frame image, do not mention duration, count only successful video generations toward the 3 videos per 24 hour quota, and upload with `--loop-video-provider gemini`. For Dreamina/Seedance, use `2.0 Fast`, first-frame only, no Omni Reference, no last-frame reference, `16:9`, `720p`, and exactly `7 seconds` through UI controls, then upload with `--loop-video-provider dreamina` or `--loop-video-provider seedance`. If the duration control is hidden on entry, drag/scroll the settings/control row to the right until the control is visible before Generate. Exception: `불송` uses `2.0 Fast`, `720p`, exactly `6 seconds`; HaruHaru photorealistic releases use Seedance `2.0`, `1080p`, exactly `7 seconds`, and final render `--video-render-resolution 1080p`. Do not put those settings in the prompt.
