...
- For `불송`, use the Buddhist profile for Buddhist scripture-inspired modern vocal music. Korean lyrics are the default, and the app schedules uploads public every other day at 07:00 Asia/Seoul.
...
- The cover should be the clean channel/requested scene with only the selected channel name as a large lower-left brand label. The YouTube thumbnail should usually use the same composition plus large readable click text and channel branding. Exception: `불송` reuses the clean cover as the thumbnail by default and should pass `--allow-cover-as-thumbnail`.
...
- For `불송`, do not make a separate heavy text thumbnail by default. Reuse the clean photorealistic Buddhist/dharma cover as the thumbnail and pass `--allow-cover-as-thumbnail`; keep only the integrated `불송` brand label unless the human explicitly asks for a text thumbnail.
...
- Use `불송` for Buddhist scripture-inspired vocal music, Dhammapada/법구경-inspired songs, Heart Sutra/반야심경-inspired songs, Buddhist jazz, Buddhist hip-hop, Buddhist R&B/soul, dharma songs, mindfulness songs, and modern sutra-inspired music. The app schedules these uploads public every other day at 07:00 Asia/Seoul.
