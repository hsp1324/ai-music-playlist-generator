...
- Do not request or accept fade-out endings for Suno playlist tracks. Prompts, style strings, vocal lyrics, and bracketed instrumental metatags should say `no fade-out`, `complete ending`, `final cadence`, `resolved ending`, or `hard ending` where appropriate. A slightly longer complete song is preferred over a track that fades out mid-idea.
