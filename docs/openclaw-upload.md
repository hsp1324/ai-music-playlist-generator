...
- For `불송`, the cover/first-frame and loop video are textless. Do not put `불송`, title text, sutra text, captions, UI, logos, or any other words in the video visual. If a YouTube thumbnail needs text, make it as a separate derivative from the textless cover and never use that thumbnail as the loop-video first frame.
...
- For `불송`, keep the video cover/first-frame textless. If thumbnail text is needed, make a separate thumbnail derivative from the textless cover and never pass that thumbnail as `--cover` or use it as the loop-video first frame. Do not use `--allow-cover-as-thumbnail` if the thumbnail has text.
...
- Use the cover or a separate first-frame image with only the lower-left channel brand label for Dreamina/Seedance/Gemini video generation. For `불송`, use a fully textless cover/first-frame. Do not use the final text thumbnail as the first-frame reference; generated video often makes large thumbnail text flicker, disappear, or reappear.
