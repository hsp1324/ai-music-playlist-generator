...
- For `Club Bloom`, use the Club Bloom profile for no-vocal EDM, house, techno, trance, festival, workout, night-drive, gaming, club, and party-energy releases. Each playlist must choose one club style lane and stay within it. Unless the human asks otherwise, Club Bloom visuals should look like a beautiful adult female DJ/BJ or club-streamer performance moment in a premium dance venue, with bold revealing club fashion and sexy nightlife energy, not abstract neon.
...
- Keep every static visual animated, anime, illustrated, or stylized unless the selected channel profile says otherwise. HaruHaru, sundaze, Solwave Radio, Cinematic Pulse, and 불송 are photorealistic/premium cinematic-real exceptions; HaruHaru, sundaze, and Solwave Radio are photorealistic lifestyle still-image by default, with sundaze/Solwave using a casual friend-taken smartphone look.
- The cover should be the clean requested scene without channel names. The YouTube thumbnail should usually use the same composition plus large readable click text, except HaruHaru where it should usually stay text-free. sundaze and Solwave Radio should keep a photorealistic friend-taken phone-photo lifestyle look and use at most one short integrated lane phrase.
...
- For `Club Bloom`, reject visual assets that are too mild for a club channel: calm abstract neon, empty low-energy backgrounds, polite lounge imagery, low-contrast thumbnails, random glowing objects, or loop videos with only tiny motion. The cover, thumbnail, and short loop video should usually show a beautiful adult female DJ/BJ with visible decks/mixer and active performance energy in a premium venue such as a beach-club deck, rooftop skyline DJ set, packed nightclub booth, concert/festival stage, warehouse rave, pool-party deck, open-air stage, yacht/harbor party, neon city terrace, or cyber club before upload. Revealing club outfits are preferred, but reject full nudity, exposed nipples/genitals, sexual acts, minors, fetish framing, celebrity likenesses, protected brands, or porn-style composition.
...
- Normal moving-video auto-publish must include a provider-generated `--loop-video` from Gemini, Dreamina, or Seedance. Do not use the thumbnail image, a text-heavy image, an app-rendered still image, or a locally synthesized motion loop as the moving video visual. A still-image fallback is allowed only when the channel profile requires it, as HaruHaru, sundaze, and Solwave Radio now do, or when the human explicitly requests it, and then OpenClaw must pass `--allow-still-image-video`. For HaruHaru, sundaze, and Solwave Radio, queue `--video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`; the MP4 uses lower-left lyrics and lower-right spectrum. `Cinematic Pulse` also uses this loop-video path by default; use photorealistic cinematic first-frame art, a subtle provider loop, `--video-render-source-mode loop_video`, `--video-render-resolution 720p`, and `--video-spectrum-overlay-style bars` unless a human asks otherwise.
...
- Use Gemini only for moving loop-video generation, not for static cover or YouTube thumbnail creation. HaruHaru, sundaze, and Solwave Radio skip this workflow in normal automation.
...
- If Gemini quota is already exhausted, do not use `--allow-still-image-video` unless the channel profile requires it, as HaruHaru, sundaze, and Solwave Radio now do, or the human explicitly approves it. Leave moving-video releases unreleased/unrendered, post `scripts/openclaw-release slack-notify --text "Gemini 3개 영상 쿼터가 끝났고 Dreamina/Seedance도 실패해서 이 릴리즈의 loop video를 보류합니다. 24시간 쿨다운 후 Gemini로 먼저 다시 만들겠습니다. RELEASE_TITLE"`, and resume that release first when Gemini can make videos again.

Dreamina website workflow for OpenClaw:

- Use `https://dreamina.capcut.com/ai-tool/home/` for browser-based Dreamina/Seedance generation on moving-video channels. Do not use it for normal HaruHaru, sundaze, or Solwave Radio releases.
...
- For HaruHaru, sundaze, and Solwave Radio photorealistic still images, visible adult faces are allowed and preferred when natural. Do not hide faces by default. Reject/regenerate only for explicit, minor-coded, celebrity-like, policy-blocked, badly distorted, or visually unusable results.
...
- a separate YouTube thumbnail image with readable text for normal moving-video channels; for HaruHaru, use a text-free photorealistic lifestyle thumbnail; for Solwave Radio, use a photorealistic friend-taken phone-photo lifestyle thumbnail with one short integrated Latin/Spanish lane phrase when useful; for `불송`, reuse the same cover/first-frame package as the thumbnail
- one short Gemini-first loop video for moving-video channels; Seedance/Dreamina clips use `1.0 Fast`, first-frame only, `720p`, exactly 10 seconds, and Gemini clips are uploaded as generated. For HaruHaru, sundaze, and Solwave Radio, skip loop video and use `still_image` render flags.
