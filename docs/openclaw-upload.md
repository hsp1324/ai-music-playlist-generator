...
- For lyric/vocal tracks, `--style` must preserve the intended lead vocal tone, timbre, delivery, register, or attitude used in Suno, not just genre and BPM. If the captured Suno style is too generic, append a concise voice phrase before upload, such as `breathy intimate female alto`, `clear bright male tenor`, `raspy low-register rap vocal`, `soft nasal indie vocal`, `warm soulful duet`, `restrained spoken-rap baritone`, `airy hook vocal`, or `confident close-mic rap delivery`. Vary that voice phrase across tracks in the same release so reused/backfilled songs can later be matched by actual vocal character.
