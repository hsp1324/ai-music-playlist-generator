...
- For `The Old Verse`, use the Old Verse profile for lyric-based Old Testament and New Testament scripture-inspired songs. Both Bible branches upload to `The Old Verse`; reserve Old Testament with `--channel-title "The Old Verse"` and New Testament with `--channel-title "New Testament"`.
- For `The New Verse`, use the New Verse profile for Buddhist scripture-inspired modern vocal music. Korean lyrics are the default, and uploads should remain private until the human reviews the new channel direction.
...
- For `The Old Verse`, use thumbnail wording such as `GENESIS SONGS`, `OLD TESTAMENT`, `NEW TESTAMENT`, `GOSPEL SONGS`, `BIBLE MUSIC`, `PSALMS MUSIC`, `SCRIPTURE SONGS`, or `EXODUS MUSIC`, with smaller `THE OLD VERSE` branding.
- For `The New Verse`, use thumbnail wording such as `BUDDHIST JAZZ`, `DHARMA R&B`, `MINDFUL HIP-HOP`, `SUTRA SONGS`, `불경 노래`, `법구경 힙합`, or `반야심경 R&B`, with smaller `THE NEW VERSE` branding until the channel is renamed.
...
- Use Korean as the default upload metadata for Tokyo/Soft Hour/HaruHaru and Buddhist `The New Verse` unless the channel profile says otherwise. Use `--default-language en` for `Storylight OST`, `Cinematic Pulse`, `Club Bloom`, `The Old Verse`, and `sundaze`; use `--default-language es` for `Solwave Radio`.
...
- Use `The Old Verse` for Old Testament, New Testament, Genesis, Matthew, Gospel, Jesus words, Psalms, Bible verse music, scripture-inspired worship, grace music, worship pop, or ancient biblical music concepts.
- Use `The New Verse` for Buddhist scripture-inspired vocal music, Dhammapada/법구경-inspired songs, Heart Sutra/반야심경-inspired songs, Buddhist jazz, Buddhist hip-hop, Buddhist R&B/soul, dharma songs, mindfulness songs, and modern sutra-inspired music. Keep these uploads private for now.
...
- For Bible releases on `The Old Verse`, use `docs/openclaw-scripture-sequence.md` and the app-owned `scripts/openclaw-release openclaw-scripture-reserve` / `openclaw-scripture-complete` helpers. Do not use a local scripture ledger or local mismatch checks during normal automation. `The New Verse` Buddhist releases do not use the Bible scripture ledger.
...
- If `AIMP_YOUTUBE_SCHEDULE_PUBLIC_ENABLED=true`, the app uploads with YouTube `status.publishAt` for the next free daily slot in `AIMP_YOUTUBE_SCHEDULE_TIMEZONE` for the selected channel. Bible releases on `The Old Verse` have two separate daily slots: Old Testament at 07:00 and New Testament at 16:00. `The New Verse` Buddhist uploads are private for now, so the app skips scheduled-public slots for that channel. Other channels' schedules do not block this channel. OpenClaw should not manually change YouTube visibility in Studio.
...
- For Gemini, use the Gemini-first `Create image` / image+prompt workflow above, attach the cover/first-frame image, do not mention duration, count only successful video generations toward the 3 videos per 24 hour quota, and upload with `--loop-video-provider gemini`. For Dreamina/Seedance, use `2.0 Fast`, first-frame only, no Omni Reference, no last-frame reference, `16:9`, `720p`, and exactly `6 seconds` through UI controls, then upload with `--loop-video-provider dreamina` or `--loop-video-provider seedance`. Exception: HaruHaru photorealistic releases use Seedance `2.0`, `1080p`, exactly `6 seconds`, and final render `--video-render-resolution 1080p`. Do not put those settings in the prompt.
