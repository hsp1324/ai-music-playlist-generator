    ...
    def _build_prompt(self, playlist: Playlist, tracks: list[Track], *, default_language: str) -> str:
        ...
        return "\n".join(
            [
                ...
                "- For 불송 Buddhist releases, use Korean default metadata, include the exact verified Buddhist source/chapter/section in the title when known, and otherwise include the verified Buddhist theme directly in the title. Do not use generic 불경 wording alone when a source/theme is available, and do not invent chapter/verse coverage. Korean/default titles should not waste title space on redundant language labels such as 한국어 랩 or 한국어 힙합; use source/theme plus the real lane instead, such as 반야심경 랩, 금강경 힙합, 법구경 힙합, 정어와 구업 힙합, 불교 힙합, or 불경 힙합. By default name one coherent Buddhist hip-hop/rap lane such as 불교 힙합, 불경 힙합, mindful hip-hop, Korean Buddhist rap, mellow boom bap, Buddhist hip-hop soul, or restrained Buddhist trap-soul. Use a non-hip-hop Buddhist lane only when the human explicitly asks or when finishing an already-started release in that lane. Do not relabel an already-rendered non-hip-hop 불송 video as hip-hop if the audio and burned-in visual text already say Dharma pop, 다르마팝, acoustic Dharma songs, or 다르마송. State that lyrics are original paraphrases inspired by Buddhist teaching, not direct scripture recitation.",
