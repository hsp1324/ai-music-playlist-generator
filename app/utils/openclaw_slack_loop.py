    ...
    *,
    ...
    previous_context.extend(
        [
            ...
            "- 가능하면 VM의 http://127.0.0.1:8000/api 또는 그 VM FastAPI 백엔드 터널을 사용하세요. 공개 sslip.io API는 Google 로그인 보호 때문에 AIMP_API_COOKIE가 있을 때만 helper upload/publish에 사용할 수 있고, AIMP_OPENCLAW_SHARED_TOKEN만으로는 부족합니다.",
            ...
            "- 그 다음 docs/openclaw-skills.md, docs/openclaw-channel-concepts/README.md, docs/openclaw-channel-profiles/README.md, docs/openclaw-visual-assets.md, docs/openclaw-youtube-metadata.md를 따라줘.",
...
def build_backlog_queue_request_message(
    *,
    ...
    lines = [
        ...
        "- 자동화 중에는 /youtube/connect, /api/youtube/connect, Google OAuth, YouTube Studio를 열지 마세요. 이미 연결된 /youtube/status channels만 사용하고 publish helper에 명시적 channel title/id를 넘기세요.",
        "- 먼저 docs/openclaw-backlog-queue.md를 읽고 그대로 따라줘.",
        "- 그 다음 docs/openclaw-next-release-planner.md, docs/openclaw-skills.md, docs/openclaw-channel-concepts/README.md, docs/openclaw-channel-profiles/README.md, docs/openclaw-visual-assets.md, docs/openclaw-youtube-metadata.md를 따라줘.",
        "- 작업 시작 시 scripts/openclaw-release openclaw-lock-start를 호출하고, 작업 중에는 scripts/openclaw-release openclaw-lock-heartbeat를 1-2분마다 호출해줘.",
        "- 작업 완료/중단 시 scripts/openclaw-release openclaw-lock-finish를 호출해줘.",
