...
- Published releases show `Request Next Playlist`, which posts a Slack command into the configured OpenClaw channel with the `AIMP_OPENCLAW_SLACK_TRIGGER_PREFIX` prefix. Set `AIMP_OPENCLAW_AUTO_REQUEST_NEXT_ON_PUBLISH=true` only when you intentionally want publish completion to trigger the next playlist automatically. OpenClaw's Slack listener should ignore ordinary channel messages and only execute messages that start with this prefix or explicitly mention the OpenClaw bot.
