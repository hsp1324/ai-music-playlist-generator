...
- Normal automated Playlist Releases now save Suno credits by asking OpenClaw for roughly 15 minutes of new approved audio on non-scripture channels, then the app tries to build a roughly 40 minute base block by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube releases. BibliaCanto and 불송 are exceptions: OpenClaw should still create roughly 40 minutes of new passage-based audio, and the app must not backfill them with other scripture/Buddhist tracks. The optional final-video repeat feature is kept behind `AIMP_PLAYLIST_FINAL_VIDEO_REPEAT_ENABLED=false` during the trial period, so current render workers can keep rendering the base block without an update.
...
Track uploads can include optional `lyrics` content. The app stores it in track metadata and exposes it in release context so OpenClaw can later create better thumbnails, moving visuals or still-image render packages, metadata, or standalone single releases from strong playlist tracks. BGM/background/study/sleep/cafe production defaults to instrumental music, but OpenClaw should use the bracket-only Suno v5.5 instrumental format in `docs/suno-v55-instrumental-format.md` instead of plain lyrics prose. Soft Hour Radio is piano-first by default: generate the first roughly 15 minutes as new solo piano/felt piano/quiet piano tracks, keep them at the front of the render, and let the app prefer previous similar Soft Hour solo-piano tracks for the back half. If there are not enough piano tracks to approach one hour, the app may fill the remaining back half with similar existing Soft Hour music instead of blocking. J-pop/K-pop/pop/Japanese pop/anime-pop production defaults to vocal songs with lyrics, so OpenClaw should upload lyrics for every pop-family track unless the human explicitly requested instrumental/no-vocal music.
