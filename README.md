...
- The next-release loop is planned by `docs/openclaw-next-release-planner.md`: it reads `/youtube/status`, rotates every connected YouTube channel, reads the selected channel's concept planner/profile in `docs/openclaw-channel-concepts/` and `docs/openclaw-channel-profiles/`, avoids recent concept repetition, and then hands off to the automatic private playlist publisher. New connected channels enter rotation automatically; if no dedicated docs exist, the planner uses the `custom-channel` fallback docs.
