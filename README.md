...
Track uploads can include optional `lyrics` content. The app stores it in track metadata and exposes it in release context so OpenClaw can later create better thumbnails, moving visuals or still-image render packages, metadata, or standalone single releases from strong playlist tracks. BGM/background/lofi/study/sleep/cafe production defaults to instrumental music, but OpenClaw should use the bracket-only Suno v5.5 instrumental format in `docs/suno-v55-instrumental-format.md` instead of plain lyrics prose. J-pop/K-pop/pop/Japanese pop/anime-pop production defaults to vocal songs with lyrics, so OpenClaw should upload lyrics for every pop-family track unless the human explicitly requested instrumental/no-vocal music.
For no-vocal Suno work, OpenClaw should also fill Suno Advanced Options excluded styles with vocal-related terms such as vocals, voice, singing, humming, choir, spoken word, narration, rap, ad-libs, scat, vocal chops, and lyrics. Playlist tracks are capped at 4:20 by default in the OpenClaw helper and must be at least 2:00 unless the human explicitly approves a shorter track. Do not put duration caps like `less than 4 minutes` into Suno prompts by default; ask for a complete song/cue with a natural intro, developed middle, and resolved ending instead.
...
The app repeats short clips internally, so OpenClaw should upload only the short source clip, not a one-hour rendered video. The renderer uses the actual uploaded clip length, normally 7 seconds; `불송` uses 6 seconds. Do not generate a draft/test clip first. Re-check the Dreamina/Seedance duration control immediately before clicking Generate, and verify the downloaded MP4 duration before upload. Do not force Dreamina to use a matching last-frame reference; first-frame-only input gives more natural motion. The cover/first-frame must not contain channel names or logos. If the first frame contains a style, genre, use-case, or passage phrase, instruct Dreamina/Seedance to keep that phrase stable only if it is already present, and reject/regenerate if the clip invents a channel label or changes the image into a different layout. Use animated, anime, illustrated, or stylized visuals instead of photorealistic/live-action footage unless the selected channel profile explicitly says otherwise. `HaruHaru` uses photorealistic daily-life still images by default: visible adult faces are allowed, but prefer natural side/three-quarter/candid friend-taken snapshots and medium/farther framing over centered face-filling AI portraits. `Solwave Radio` uses photorealistic Latin/Spanish daily-life still images by default: visible adult faces are allowed, but prefer casual friend-taken night-out/travel/cafe phone snapshots over professional photographer shoots, studio portraits, glossy fashion campaigns, or face-filling AI beauty portraits. `Cinematic Pulse` uses original photorealistic cinematic film-still / premium movie-poster realism with clean `bars` spectrum renders.
