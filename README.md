...
- OpenClaw full auto-publish runs require a final uploaded 16:9 video cover/first-frame and a separate 16:9 YouTube thumbnail. Most channels use readable click text on the thumbnail; HaruHaru should normally use text-free photorealistic lifestyle cover/thumbnail images, now biased toward friend-taken adult Hongdae/Seoul street snapshots with hip streetwear, slight motion softness, and a Korean hip-hop/R&B mood rather than tight straight-on AI-model or idol-pop portraits. Solwave Radio should use photorealistic Latin/Spanish friend-taken smartphone/Instagram-style cover/thumbnail images; the thumbnail may add one short integrated Latin/Spanish lane phrase when useful. `불송` should use one respectful photorealistic Buddha-reading-sutra package for cover, thumbnail, first-frame, and loop-video first frame: warm gentle expression, subtle music-listening cue, open Buddhist sutra, and calm temple/meditation setting. Do not put the YouTube channel name, channel logo, or brand label on covers, thumbnails, first-frame images, or loop videos. If text is useful, use a short natural style, genre, use-case, or passage phrase such as `J-POP`, `LOFI`, `TECH HOUSE`, `CINEMATIC ORCHESTRA`, `Genesis 1:1-5`, or `팔정도 명상팝`; integrate it into the artwork without a detached badge or hard box. Moving-video runs must include the configured short Gemini/Dreamina/Seedance MP4 with `--loop-video`; the app repeats that short clip during final video render. HaruHaru and Solwave Radio instead render from the still cover using `--allow-still-image-video --video-render-source-mode still_image --video-render-resolution 1080p --video-spectrum-overlay-style bars --lyrics-overlay --lyrics-overlay-style editorial-lower-left`. The local draft cover is a manual placeholder and is not used for automatic YouTube publishing unless explicitly allowed.
...
- Normal automated Playlist Releases now save Suno credits by asking OpenClaw for roughly 10 minutes of new approved audio on non-scripture channels, then the app tries to build a roughly 40-60 minute base block by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube releases. BibliaCanto and 불송 are exceptions: OpenClaw should still create roughly 40 minutes of new passage-based audio, and the app must not backfill them with other scripture/Buddhist tracks. The optional final-video repeat feature is kept behind `AIMP_PLAYLIST_FINAL_VIDEO_REPEAT_ENABLED=false` during the trial period, so current render workers can keep rendering the base block without an update.
