...
- Track intake, indexed search suggestion, and decision APIs
...
- `AIMP_SLACK_IMAGE_UPLOAD_MAX_EDGE`
...
- OpenClaw full auto-publish runs require a final uploaded 16:9 video cover/first-frame and a separate 16:9 YouTube thumbnail. Most channels use readable click text on the thumbnail; HaruHaru should normally use text-free photorealistic lifestyle cover/thumbnail images, now biased toward friend-taken adult Hongdae/Seoul street snapshots with hip streetwear, slight motion softness, and a Korean hip-hop/R&B mood rather than tight straight-on AI-model or idol-pop portraits. Solwave Radio should use photorealistic Latin/Spanish friend-taken smartphone/Instagram-style cover/thumbnail images; the thumbnail may add one short integrated Latin/Spanish lane phrase when useful. `불송` should use one respectful text-free Buddhist music image package for cover, thumbnail, and first-frame, but must vary aggressively across releases: photorealistic street-Buddhist scenes, surreal/metaphysical symbolism, cubist-inspired collage, new-age lotus abstraction, cyber-Buddhist neon, anime/illustrated Buddhist fantasy, dark editorial fashion, object-led album art, or other fresh lanes. Put scripture/source/genre phrases such as `법구경 힙합` in YouTube metadata unless the human explicitly asks for text in the image. Do not put the YouTube channel name, channel logo, or brand label on covers, thumbnails, first-frame images, or loop videos. If text is useful for other channels, use a short natural style, genre, use-case, or passage phrase such as `J-POP`, `LOFI`, `TECH HOUSE`, `CINEMATIC ORCHESTRA`, or `Genesis 1:1-5`; integrate it into the artwork without a detached badge or hard box. Moving-video runs must include the configured short Gemini/Dreamina/Seedance MP4 with `--loop-video`; the app repeats that short clip during final video render. HaruHaru, sundaze, Solwave Radio, Club Bloom, and 불송 instead render from the still cover using channel-specific `--allow-still-image-video --video-render-source-mode still_image` flags. The local draft cover is a manual placeholder and is not used for automatic YouTube publishing unless explicitly allowed.
- The web release detail view exposes separate upload/replace buttons for video cover, YouTube thumbnail, and short loop video.
...
- If the playlist has a rendered local audio file and YouTube is connected, publish approval will queue a background job that:
  ...
  - uses the uploaded short loop video as the moving visual when `--loop-video` is provided
...
- Normal automation can offload video rendering to external render workers. Set `AIMP_VIDEO_RENDER_EXECUTION_MODE=external` and `AIMP_RENDER_WORKER_SHARED_TOKEN`, then run `scripts/render-worker` on another machine. The main VM keeps DB/UI/YouTube ownership; the render worker claims queued video jobs, renders locally, and uploads the final MP4 back with resumable chunks. Worker profiles affect claim priority: `--worker-profile desktop` prefers 1080p/2k jobs, while `--worker-profile oracle` prefers 720p jobs. See `docs/external-video-render-worker.md`.
- Operational Slack notices use `AIMP_SLACK_OPS_CHANNEL_ID`; production must set it to `#all-ai-music-playlist-generator` / `C0ATYMCMLLE`, not the OpenClaw command channel. The app posts clean video-render queued, render-worker claim, render-worker complete, render-worker heartbeat-timeout requeue, and YouTube publish-complete notices there, attaching the release thumbnail/cover when available and omitting internal job IDs from visible Slack text. Attached artwork is uploaded as a small Slack preview with `AIMP_SLACK_IMAGE_UPLOAD_MAX_EDGE` defaulting to `480`. It does not post a Slack notice merely because a playlist reached its target duration.
- After a successful YouTube upload, the app immediately deletes the long final rendered MP4 from local storage, records `local_video_deleted_after_youtube_upload` in release metadata, and clears `output_video_path`. It keeps source assets such as audio, cover, thumbnail, and any short Gemini/Dreamina/Seedance loop video so visual fixes and re-renders remain possible.
- Local rendered MP4 cleanup is also enabled as a fallback. When disk usage for `AIMP_STORAGE_ROOT` rises above `AIMP_LOCAL_VIDEO_CLEANUP_DISK_THRESHOLD_PERCENT` (default `80`), the app deletes any remaining local final rendered MP4 files for releases that were already uploaded to YouTube and are public, oldest first, then records the deletion in release metadata and clears `output_video_path`. This fallback does not delete short loop/source videos.
...
- Normal automated Playlist Releases now save Suno credits by asking OpenClaw for roughly 10 minutes of new approved audio on normal playlist channels and 불송, then the app tries to build a roughly 40-60 minute base block by reusing previous same-channel, similar-genre tracks from the back half of already uploaded YouTube releases. 불송 is a strict-lane exception inside that reuse path: it can backfill only from previous same-channel, same-detailed-lane Buddhist material, liked first, with reused tracks kept after the fresh lead block. BibliaCanto remains excluded from back-half reuse and should create new passage-based audio. The optional final-video repeat feature is kept behind `AIMP_PLAYLIST_FINAL_VIDEO_REPEAT_ENABLED=false` during the trial period, so current render workers can keep rendering the base block without an update.
...
Track uploads can include optional `lyrics` content. The app stores it in track metadata and exposes it in release context so OpenClaw can later create better thumbnails, moving visuals or still-image render packages, metadata, or standalone single releases from strong playlist tracks. BGM/background/study/sleep/cafe production defaults to instrumental music, but OpenClaw should use the bracket-only Suno v5.5 instrumental format in `docs/suno-v55-instrumental-format.md` instead of plain lyrics prose. Soft Hour Radio is piano-first by default: generate the first roughly 10 minutes as new solo piano/felt piano/quiet piano tracks, keep them at the front of the render, and let the app prefer previous similar Soft Hour solo-piano tracks for the back half. If there are not enough piano tracks to approach one hour, the app may fill the remaining back half with similar existing Soft Hour music instead of blocking. J-pop/K-pop/pop/Japanese pop/anime-pop production defaults to vocal songs with lyrics, so OpenClaw should upload lyrics for every pop-family track unless the human explicitly requested instrumental/no-vocal music.
Starting 2026-08-24, every new song with singable lyrics follows `docs/openclaw-vocal-arrangement-policy.md`: one playlist uses exactly one of themed large-scale orchestral vocal, piano-only vocal, or acoustic-guitar-only vocal. Orchestral releases are the primary differentiator and each one chooses a coherent theme such as epic, lyrical, majestic, Nordic, medieval, dark-fantasy, mythic, romantic, or celestial. Existing generated assets are preserved, but new vocal reuse must match the same family and compatible orchestral theme; render shorter rather than filling with legacy beat-led or unrelated tracks.
For no-vocal Suno work, OpenClaw should also fill Suno Advanced Options excluded styles with vocal-related terms such as vocals, voice, singing, humming, choir, spoken word, narration, rap, ad-libs, scat, vocal chops, and lyrics. Playlist tracks have no default maximum duration in the OpenClaw helper; tracks below 1:00 need explicit human approval before use; tracks from 1:00 to 1:59 are accepted and recorded for later analysis. Do not put duration caps or two-minute lower-bound wording into Suno prompts by default; ask for an around 4 minute complete song/cue with a natural intro, developed middle, and resolved ending instead.
...
`AIMP_DREAMINA_VIDEO_MODEL` is the provider model identifier. `AIMP_DREAMINA_VIDEO_DURATION_SECONDS=10` is only the requested clip length in seconds, not a model version.
...
For HaruHaru and Solwave Radio, do not upload a loop video during normal automation; queue:
...
The app repeats short clips internally, so OpenClaw should upload only the short source clip, not a one-hour rendered video. The renderer uses the actual uploaded clip length, normally 7 seconds. Do not generate a draft/test clip first. Re-check the Dreamina/Seedance duration control immediately before clicking Generate, and verify the downloaded MP4 duration before upload. Do not force Dreamina to use a matching last-frame reference; first-frame-only input gives more natural motion. The cover/first-frame must not contain channel names or logos. If the first frame contains a style, genre, use-case, or passage phrase, instruct Dreamina/Seedance to keep that phrase stable only if it is already present, and reject/regenerate if the clip invents a channel label or changes the image into a different layout. Use animated, anime, illustrated, or stylized visuals instead of photorealistic/live-action footage unless the selected channel profile explicitly says otherwise. `HaruHaru` uses photorealistic Korean hip-hop/R&B street still images by default: visible adult faces are allowed, but prefer natural side/three-quarter/candid friend-taken Hongdae/Seoul street snapshots, hip adult streetwear, medium/farther framing, slight motion blur or focus shake, and visible street context over centered face-filling AI or idol portraits. `Solwave Radio` uses photorealistic Latin/Spanish daily-life still images by default: visible adult faces are allowed, but prefer casual friend-taken night-out/travel/cafe phone snapshots over professional photographer shoots, studio portraits, glossy fashion campaigns, or face-filling AI beauty portraits. `불송` uses app-rendered still images by default, with diversity-first Buddhist music visuals and no provider loop video unless the human explicitly changes that standing rule. `Cinematic Pulse` uses original photorealistic cinematic film-still / premium movie-poster realism with clean `bars` spectrum renders.

Video-generation prompts should describe the visible state, not the conceptual reason for the release. Avoid words that cause provider models to stage cuts or YouTube-like edits, such as `playlist`, `music visual`, `seamless loop`, `repeat`, `cyclic`, or scripture framework terms that imply multiple narrative stages. For example, for a Buddhist release do not prompt `Four Noble Truths playlist`; prompt the actual visible scene and mood instead. Prefer positive fixed-shot language such as `single fixed camera shot`, `locked-off camera`, `one uninterrupted calm environmental take`, and `same composition from first to last frame`. Keep negative lists short; too many `do not` clauses can make video providers focus on the forbidden action.

For `Cinematic Pulse`, create a subtle provider loop video during normal automation from high-resolution photorealistic 16:9 cover/first-frame art, preferably 2560x1440 or 1920x1080, then queue:

```bash
scripts/openclaw-release render-video --release-id RELEASE_ID --video-render-source-mode loop_video --video-render-resolution 720p --video-spectrum-overlay-style bars
```
