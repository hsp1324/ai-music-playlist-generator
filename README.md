...
- Video rendering can be distributed across extra machines with `scripts/render-worker`; see `docs/distributed-render-workers.md`. External workers poll the app, claim queued video jobs, render locally with ffmpeg, upload the MP4 back, and stale claims are requeued after one day by default.
